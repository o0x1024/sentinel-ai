...
function createSnapshot(seed: number): TrafficPluginRuntimeQueueStatsSnapshot {
  const stats = (kind: TrafficPluginRuntimeQueueStatsSnapshot['activeProbe']['kind']) => ({
    kind,
    pendingCount: seed,
    queuedCount: 0,
    scheduledCount: 0,
  ...
  return {
    activeProbe: stats('traffic_active_probe'),
    bountyFetch: stats('bounty_fetch'),
    monitorFetch: stats('monitor_fetch'),
    agentFetch: stats('agent_fetch'),
    pluginTestFetch: stats('plugin_test_fetch'),
  }
}
