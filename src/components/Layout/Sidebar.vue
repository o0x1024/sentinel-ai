          ...
          <ul class="menu menu-sm space-y-1">
            <li v-for="item in mainMenuItems" :key="item.path">
              <router-link
                ...
                :class="{
                  'opacity-50': item.proOnly,
                  'bg-primary/10 text-primary border-r-2 border-primary': !item.proOnly && (item.path.startsWith('/security-center')
                    ? isSecurityCenterRoute
                    : route.path === item.path),
                  'bg-base-300/50': item.proOnly && route.path === item.path,
                }"
              >
