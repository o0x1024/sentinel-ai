<template>
  <div class="space-y-3">
    ...
    <div v-if="showRawText" class="space-y-2">
      <p class="text-xs font-medium text-base-content/60">
        {{ entries.length ? '原始文本' : '证据片段' }}
      </p>
      <pre
        class="bg-black/80 text-green-400 p-3 rounded text-xs overflow-x-auto whitespace-pre-wrap break-words"
        >{{ evidenceSnippet }}</pre
      >
    </div>
  </div>
...
const entries = computed(() => parseSecurityEvidenceSnippetEntries(props.evidenceSnippet))
const showRawText = computed(() => Boolean((props.evidenceSnippet || '').trim()))
