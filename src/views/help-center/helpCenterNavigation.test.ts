...
describe('helpCenterNavigation', () => {
  it('builds a collapsed tree from localized help content', () => {
    ...
    expect(tree[3]?.children.map(node => node.id)).toContain('feature-bug-bounty-operations')
  ...
  it('flattens every section id for scroll tracking', () => {
    ...
    expect(ids).toContain('feature-bug-bounty-findings')
    expect(ids).toContain('feature-bug-bounty-knowledge')
