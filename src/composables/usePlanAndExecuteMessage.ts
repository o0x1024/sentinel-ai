  ...
  for (const chunk of executionChunks) {
    ...
    if (chunkType === 'ToolResult') {
      try {
        ...
        if (resultData && resultData.tool_name) {
          const stepName = resultData.step_name || resultData.tool_name
