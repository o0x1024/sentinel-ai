export default {
  title: '设置',
  language: '语言设置',
  selectLanguage: '选择语言',
  languageNote: '语言更改将立即生效，无需重启',
  aiSettings: 'AI服务设置',
  aiServiceConfig: 'AI服务配置',
  apiKeys: 'API密钥',
  providers: '服务提供商',
  models: '模型',
  advanced: '高级设置',
  ragConfig: 'RAG配置',
  ragTestConnection: '测试RAG连接',
  ragResetConfig: '重置RAG配置',
  ragSaveConfig: '保存RAG配置',
  testingConnection: '正在测试连接...',
  connectionSuccess: '连接成功',
  ragConnectionFailed: 'RAG连接失败',
  configReset: '配置已重置',
  configSaved: '配置已保存',
  performance: '性能设置',
  enabled: '已启用',
  disabled: '已禁用',
  saving: '保存中...',
  enableTenthMan: '启用第十人原则',
  interventionMode: '干预模式',
  finalOnly: '仅最终审查',
  proactive: '主动介入',
  toolCallInterval: '工具调用审查间隔',
  toolCallIntervalHint: '每N个工具调用后触发审查',
  dangerousKeywords: '危险关键词',
  dangerousKeywordsHint: '用逗号分隔多个关键词',
  requireConfirmation: '危险操作需确认',
  requireConfirmationHint: '检测到危险操作时暂停执行，等待用户确认',
  saveSuccess: '设置已保存',
  saveFailed: '保存失败',
  security: {
    title: '安全设置',
    status: '状态',
    statusGood: '良好',
    statusWarning: '警告',
    statusCritical: '严重',
    score: '评分',
    encryption: '加密',
    encryptionType: '加密类型',
    encryptionSettings: '加密设置',
    enableEncryption: '启用加密',
    enableEncryptionHint: '加密本地存储的敏感数据',
    encryptionAlgorithm: '加密算法',
    recommended: '推荐',
    keyManagement: '密钥管理',
    keyManagementAuto: '自动',
    keyManagementManual: '手动',
    keyManagementHardware: '硬件',
    keyRotation: '密钥轮换',
    rotationPeriod: '轮换周期',
    days: '天',
    encryptionScope: '加密范围',
    encryptDatabase: '数据库加密',
    encryptConfig: '配置加密',
    encryptLogs: '日志加密',
    encryptCache: '缓存加密',
    encryptBackups: '备份加密',
    authentication: '身份验证',
    basicAuth: '基础认证',
    requireAuth: '启用身份验证',
    requireAuthHint: '访问应用时需要进行身份验证',
    authMethod: '认证方式',
    authMethods: {
      password: '密码',
      pin: 'PIN',
      biometric: '生物识别',
      token: '令牌'
    },
    currentPassword: '当前密码',
    enterCurrentPassword: '请输入当前密码',
    newPassword: '新密码',
    enterNewPassword: '请输入新密码',
    confirmPassword: '确认密码',
    confirmNewPassword: '请再次输入新密码',
    changePassword: '修改密码',
    pin: 'PIN',
    enterPin: '请输入PIN',
    pinHint: '建议 4–8 位数字',
    advancedAuth: '高级认证',
    sessionTimeout: '会话超时',
    minutes: '分钟',
    sessionTimeoutHint: '无操作后自动退出登录',
    maxLoginAttempts: '最大登录尝试次数',
    maxLoginAttemptsHint: '失败次数过多将临时锁定',
    lockoutDuration: '锁定时长',
    hour: '小时',
    hours: '小时',
    twoFactorAuth: '双因素认证',
    twoFactorAuthHint: '登录时增加额外验证步骤',
    networkSecurity: '网络安全',
    connectionSecurity: '连接安全',
    forceHTTPS: '强制HTTPS',
    forceHTTPSHint: '尽可能强制使用HTTPS连接',
    verifyCertificates: '验证证书',
    useProxy: '使用代理',
    proxyType: '代理类型',
    proxyHost: '代理主机',
    proxyPort: '代理端口',
    accessControl: '访问控制',
    enableIPWhitelist: '启用IP白名单',
    allowedIPs: '允许的IP',
    allowedIPsPlaceholder: '例如：192.168.1.0/24, 10.0.0.1',
    allowedIPsHint: '使用逗号分隔多个 IP/CIDR',
    enableRateLimit: '启用限流',
    requestsPerMinute: '每分钟请求数',
    burstLimit: '突发上限',
    logSettings: '日志设置',
    logRetention: '日志保留期',
    forever: '永久',
    compressLogs: '压缩日志',
    remoteLogging: '远程日志',
    logServer: '日志服务器',
    logApiKey: 'API密钥',
    enterLogApiKey: '请输入API密钥',
    securityOperations: '安全操作',
    securityCheck: '安全检查',
    checkVulnerabilities: '检查漏洞',
    generateReport: '生成报告',
    emergencyOperations: '紧急操作',
    lockApplication: '锁定应用',
    emergencyShutdown: '紧急关机',
    wipeSecurityData: '清除安全数据',
    wipeSecurityDataWarning: '此操作不可逆，将删除本地安全相关数据',
    saveConfig: '保存安全设置',
    licenseAdmin: {
      title: '高级功能权限同步管理',
      description: '集中配置高级功能权限的服务端同步链路。普通用户不应在授权弹窗里接触这些参数。',
      refreshStatus: '刷新状态',
      adminOnlyHint: '这里只用于管理员维护本地 license 到高级功能权限的同步服务，不是终端用户的激活步骤。',
      localLicense: '本地 License',
      localLicenseActive: '已激活',
      localLicenseMissing: '未激活',
      currentToken: '当前权限令牌',
      tokenValid: '有效',
      tokenUnavailable: '不可用',
      tokenExpiresAt: '过期时间：{expiresAt}',
      tokenValidDetail: '当前权限令牌可用于高价值功能授权。',
      refreshService: '刷新服务',
      refreshConfigured: '已配置',
      refreshNotConfigured: '未配置',
      apiKeyConfigured: '刷新 API Key 已配置',
      apiKeyMissing: '刷新 API Key 尚未配置',
      lastSync: '最近同步',
      noSyncYet: '暂无成功记录',
      runtimeIdle: '当前没有新的同步错误。',
      enableAutoRefresh: '启用自动同步高级功能权限',
      timeout: '超时时间（秒）',
      endpoint: 'Refresh Endpoint',
      customerId: 'Customer ID',
      customerIdPlaceholder: '可选，用于租户或客户识别',
      apiKey: 'Refresh API Key',
      apiKeyPlaceholder: '服务端签发接口使用的 API Key / Bearer Token',
      refreshNow: '立即同步',
      save: '保存同步配置',
      saveSuccess: '同步配置已保存',
      notAvailable: '不可用',
      manualTokenTitle: '手工权限令牌工具',
      manualTokenField: '高级功能权限令牌',
      manualTokenPlaceholder: '仅在排障、支持或 release 链路验证时手工粘贴短期权限令牌（entitlement token）',
      manualTokenHint: '普通用户不应操作这里。优先使用自动同步，只有服务端链路不可用时才手工写入。',
      manualTokenStore: '写入权限令牌',
      manualTokenClear: '清除权限令牌'
    }
  },
  about: '关于',
  version: '版本',
  addProvider: '添加服务提供商',
  editProvider: '编辑服务提供商',
  deleteProvider: '删除服务提供商',
  testConnection: '测试连接',
  connectionSuccessful: '连接成功',
  connectionFailed: '连接失败',
  apiKeyPlaceholder: '输入您的API密钥',
  apiKeyRequired: 'API密钥是必需的',
  modelRequired: '必须选择模型',
  providerRequired: '必须选择服务提供商',
  general: {
    version: '版本',
    buildDate: '构建日期',
    language: '语言',
    region: '地区',
    theme: '主题',
    darkMode: '深色模式',
    interface: '界面设置',
    appearance: '外观',
    languageRegion: '语言和地区',
    fontSize: '字体大小',
    compactMode: '紧凑模式',
    compactModeHint: '减少间距和填充以获得更紧凑的界面',
    darkModeHint: '根据系统设置自动切换浅色和深色主题',
    timezone: '时区',
    dateFormat: '日期格式',
    behavior: '行为设置',
    startup: '启动设置',
    autoStart: '开机启动',
    autoStartHint: '系统启动时自动运行应用',
    startMinimized: '启动时最小化',
    restoreSession: '恢复上次会话',
    restoreSessionHint: '启动时恢复上次会话状态',
    checkUpdates: '检查更新',
    network: '网络设置',
    maxConnections: '最大并发连接数',
    maxConnectionsHint: '允许的最大并发网络连接数',
    requestTimeout: '请求超时',
    retryCount: '重试次数',
    privacy: '隐私设置',
    analytics: '数据收集',
    analyticsHint: '发送匿名使用数据以帮助改进应用',
    errorReporting: '错误报告',
    errorReportingHint: '发送崩溃/错误报告以帮助提升稳定性',
    usageStats: '使用统计',
    encryptLocalData: '本地存储加密',
    encryptLocalDataHint: '加密此设备上的本地存储数据',
    languages: {
      auto: '自动',
      zhCN: '简体中文',
      zhTW: '繁体中文',
      enUS: '英语（美国）',
      jaJP: '日语',
      koKR: '韩语',
      frFR: '法语',
      deDE: '德语',
      esES: '西班牙语',
      ptBR: '葡萄牙语（巴西）',
      ruRU: '俄语'
    },
    themes: {
      auto: '自动',
      light: '浅色',
      dark: '深色',
      cupcake: '杯子蛋糕',
      bumblebee: '大黄蜂',
      emerald: '翡翠',
      corporate: '商务',
      synthwave: '合成波',
      retro: '复古',
      cyberpunk: '赛博朋克',
      valentine: '情人节',
      halloween: '万圣节',
      garden: '花园',
      forest: '森林',
      aqua: '水蓝',
      lofi: 'Lo-fi',
      pastel: '粉彩',
      fantasy: '幻想',
      wireframe: '线框',
      black: '黑色',
      luxury: '奢华',
      dracula: '德古拉'
    },
    regions: {
      auto: '自动',
      CN: '中国',
      TW: '中国台湾',
      HK: '中国香港',
      US: '美国',
      JP: '日本',
      KR: '韩国',
      EU: '欧洲'
    },
    timezones: {
      auto: '自动',
      shanghai: '上海',
      taipei: '台北',
      hongkong: '香港',
      tokyo: '东京',
      seoul: '首尔',
      newyork: '纽约',
      losangeles: '洛杉矶',
      london: '伦敦',
      paris: '巴黎'
    },
    unknown: '未知',
    window: '窗口设置',
    closeToTray: '关闭到系统托盘',
    closeToTrayHint: '关闭窗口时在后台保持运行',
    minimizeToTray: '最小化到系统托盘',
    alwaysOnTop: '始终置顶',
    windowOpacity: '窗口透明度',
    performance: '性能',
    memory: '内存设置',
    memoryLimit: '内存限制',
    memoryLimitHint: '应用程序的最大内存使用量',
    autoGC: '自动垃圾回收',
    preload: '预加载资源',
    preloadHint: '预加载常用资源以获得更好的性能',
    enabled: '已启用',
    disabled: '已禁用',
    logLevel: '日志级别',
    logLevelHint: '控制应用程序输出日志的详细程度',
    logLevels: {
      trace: 'Trace - 最详细',
      debug: 'Debug - 调试信息',
      info: 'Info - 常规信息',
      warn: 'Warn - 警告',
      error: 'Error - 仅错误'
    }
  },
  themes: {
    light: '浅色主题',
    dark: '深色主题',
    auto: '自动（系统）',
    custom: '自定义主题',
    corporate: '企业主题',
    themeColors: '主题颜色',
    primaryColor: '主色调',
    secondaryColor: '次要颜色',
    accentColor: '强调色',
    backgroundColor: '背景色',
    textColor: '文本颜色',
    resetColors: '重置颜色',
    saveTheme: '保存主题'
  },
  system: {
    title: '系统设置',
    autoUpdate: '自动更新',
    autoUpdateDesc: '自动检查并安装更新',
    updateChannel: '更新通道',
    stable: '稳定版',
    beta: '测试版',
    dev: '开发版',
    crashReporting: '崩溃报告',
    crashReportingDesc: '发送崩溃报告以帮助改进应用程序',
    telemetry: '使用分析',
    telemetryDesc: '发送匿名使用数据以帮助改进应用程序',
    performance: '性能设置',
    startup: '启动设置',
    startWithSystem: '随系统启动',
    startMinimized: '启动时最小化',
    minimizeToTray: '最小化到系统托盘',
    closeToTray: '关闭到系统托盘',
    notifications: '通知',
    enableNotifications: '启用通知',
    notificationSound: '通知声音',
    desktopNotifications: '桌面通知',
    language: '语言设置',
    selectLanguage: '选择语言',
    languageHint: '语言更改将立即生效，无需重启',
    region: '地区设置',
    timezone: '时区',
    dateFormat: '日期格式',
    timeFormat: '时间格式'
  },
  scan: {
    title: '扫描设置',
    autoScan: '自动扫描',
    autoScanDesc: '添加新资产时自动开始扫描',
    scanInterval: '扫描间隔',
    scanIntervalDesc: '自动扫描之间的间隔',
    concurrentScans: '并发扫描',
    concurrentScansDesc: '最大并发扫描数',
    scanTimeout: '扫描超时',
    scanTimeoutDesc: '单次扫描的最大时间（分钟）',
    retryAttempts: '重试次数',
    retryAttemptsDesc: '失败扫描的重试次数',
    scanThreads: '扫描线程',
    scanThreadsDesc: '扫描的线程数',
    enableTrafficAnalysis: '启用流量分析',
    trafficAnalysisDesc: '启用流量分析模式',
    trafficAnalysisPort: '流量分析端口',
    trafficAnalysisPortDesc: '流量分析的端口'
  },
  network: {
    title: '网络设置',
    proxy: '代理设置',
    globalProxyTitle: '全局出站代理',
    globalProxyDescription: '仅影响 Sentinel 主动发起的出站请求，例如插件 fetch、模型访问和内置 HTTP 客户端。',
    enableProxy: '启用代理',
    enableGlobalProxy: '启用全局代理',
    scheme: '协议',
    host: '主机',
    port: '端口',
    noProxy: '不走代理',
    username: '用户名(可选)',
    password: '密码(可选)',
    proxySchemes: {
      http: 'HTTP - 标准HTTP代理',
      https: 'HTTPS - HTTPS代理',
      socks5: 'SOCKS5 - 本地DNS解析',
      socks5h: 'SOCKS5H - 远程DNS解析(推荐)'
    },
    proxySchemeHint: '提示: SOCKS5H更安全，DNS请求也通过代理',
    placeholders: {
      host: '127.0.0.1',
      port: '7890',
      noProxy: 'localhost,127.0.0.1'
    },
    defaults: {
      scheme: 'http',
      notAvailable: 'N/A'
    },
    proxyType: '代理类型',
    proxyHost: '代理主机',
    proxyPort: '代理端口',
    proxyUsername: '代理用户名',
    proxyPassword: '代理密码',
    gateway: {
      title: 'AI HTTP 网关',
      description: '通过 HTTP API 暴露 AI 助手（默认关闭）',
      host: '网关主机',
      port: '网关端口',
      allowLan: '允许局域网访问',
      save: '保存网关配置',
      generateApiKey: '生成 API 密钥',
      refreshStatus: '刷新状态',
      status: '网关状态',
      saved: '网关配置已保存',
      saveFailed: '保存网关配置失败: {error}',
      started: 'HTTP 网关已启动',
      stopped: 'HTTP 网关已停止',
      toggleFailed: '切换网关状态失败: {error}',
      generated: '已生成新的 API 密钥',
      generatedKeyNotice: '请立即保存该密钥，仅显示一次。',
      generateFailed: '生成 API 密钥失败: {error}'
    },
    weixin: {
      title: '微信远程控制',
      description: '通过个人微信私聊或群聊远程操控本应用（默认关闭，启用后私聊和群聊默认响应）。',
      securityHint: '远程控制会进入真实 AI 助手和工具执行链路。建议使用专用微信号；如不希望群聊触发任务，可关闭群聊开关。',
      accountId: 'Account ID',
      token: 'Token',
      baseUrl: 'iLink Base URL',
      assistantProfile: '微信 bot Agent Profile',
      defaultAssistantProfile: '默认交互型 Agent Profile',
      assistantProfileHint: '来自智能体管理的交互型 Agent Profiles；微信不接 Team Profiles。',
      allowedUsers: '私聊用户白名单',
      allowedUsersHint: '留空表示允许所有私聊用户；填写后仅允许这些用户 ID。',
      enableGroups: '允许群聊远程控制',
      maxIterations: '最大工具轮数',
      timeoutSecs: '超时（秒）',
      save: '保存微信配置',
      qrLogin: '扫码登录',
      refreshStatus: '刷新状态',
      status: '微信网关状态',
      lastMessageAt: '最近收到消息',
      saved: '微信配置已保存',
      saveFailed: '保存微信配置失败: {error}',
      started: '微信网关已启动',
      stopped: '微信网关已停止',
      toggleFailed: '切换微信网关失败: {error}',
      qrPending: '请使用微信扫码确认登录',
      qrWaiting: '等待扫码确认...',
      qrConfirmed: '微信登录已确认，凭据已保存',
      qrFailed: '创建微信二维码失败: {error}'
    },
    runtime: {
      title: '当前代理生效状态',
      description: '这里展示当前真正影响流量的代理相关功能。即使全局出站代理关闭，本地流量代理仍可能在拦截流量。',
      refresh: '刷新状态',
      globalProxy: '全局出站代理',
      localProxy: '流量分析代理',
      active: '生效中',
      inactive: '未生效',
      unknown: '未知',
      notSupported: '当前平台不支持',
      globalProxyActiveDetail: 'Sentinel 主动发起的 HTTP 客户端请求会走已配置代理。',
      globalProxyInactiveDetail: 'Sentinel 主动发起的 HTTP 客户端将直连，除非另有流量捕获机制开启。',
      localProxyActiveDetail: '本地监听代理正在端口 {port} 运行，请求可能会出现在代理历史中。',
      localProxyInactiveDetail: '流量分析代理监听器未运行。',
      localProxyAutoStartOn: '自动启动：已开启',
      localProxyAutoStartOff: '自动启动：已关闭',
      confusionHint: '如果你看到请求“像是走了代理”，优先检查下面的流量分析代理状态。'
    },
    toast: {
      proxySaved: '全局代理已保存并生效',
      proxySaveFailed: '保存全局代理失败',
      runtimeStatusFailed: '刷新当前代理状态失败'
    },
    timeout: '网络超时',
    timeoutDesc: '网络请求超时时间（秒）',
    retryAttempts: '重试次数',
    retryAttemptsDesc: '网络请求的重试次数',
    userAgent: '用户代理',
    userAgentDesc: 'HTTP请求的自定义用户代理',
    headers: '自定义头部',
    headersDesc: '自定义HTTP头部'
  },
  categories: {
    ai: 'AI服务',
    rag: '知识库配置',
    scheduler: '模型配置',
    agent: 'Agent',
    database: '数据库',
    system: '系统设置',
    security: '安全设置',
    network: '网络设置'
  },
  agent: {
    title: 'Agent设置',
    autoSaveFailed: '自动保存失败',
    terminal: {
      title: '终端设置',
      autoExecution: '终端命令自动执行',
      autoExecutionDesc: '控制 Agent 执行终端命令前是否需要用户确认',
      alwaysProceed: '始终执行',
      requestReview: '请求审核',
      allowList: '允许列表',
      allowListDesc: 'Agent 会自动执行匹配允许列表条目的命令',
      denyList: '拒绝列表',
      denyListDesc: 'Agent 执行匹配拒绝列表条目的命令前会请求权限',
      enterCommand: '输入命令...',
      noAllowedCommands: '允许列表为空',
      noDeniedCommands: '拒绝列表为空',
      dockerImage: 'Docker 镜像',
      dockerImageDesc: '交互式终端使用的 Docker 镜像',
      dockerImagePlaceholder: '例如: sentinel-sandbox:latest',
      useDocker: '使用 Docker',
      useDockerDesc: '使用 Docker 容器作为终端环境（推荐），关闭后将使用主机终端',
      useHostNetwork: '使用主机网络',
      useHostNetworkDesc: '开启后容器将使用 host 网络模式；关闭时使用 bridge 网络（默认）',
      hostShell: 'Host 默认 Shell',
      hostShellDesc: '主机运行环境启动交互式终端时使用的 shell，必须是可执行文件的绝对路径',
      hostShellPlaceholder: '例如: /bin/zsh',
      dockerShell: 'Docker 默认 Shell',
      dockerShellDesc: 'Docker 容器内启动交互式终端时使用的 shell 命令',
      dockerShellPlaceholder: '例如: bash',
      memoryLimit: '内存限制',
      memoryLimitDesc: '容器内存限制，例如 512m、1g、2g',
      memoryLimitPlaceholder: '例如: 2g',
      cpuLimit: 'CPU 限制',
      cpuLimitDesc: '容器 CPU 核心数限制，例如 1.0、2.0、4.0',
      cpuLimitPlaceholder: '例如: 4.0'
    },
    permissionHistory: {
      title: 'Shell 权限历史',
      desc: '查看最近的 shell 审批决策、语义分类，以及持久化的 allow rule。',
      filtersTitle: '筛选条件',
      refresh: '刷新',
      applyFilters: '应用筛选',
      clearFilters: '清空筛选',
      loading: '正在加载 shell 权限历史...',
      empty: '暂时还没有 shell 权限审批记录。',
      emptyFiltered: '当前筛选条件下没有匹配记录。',
      loadMore: '加载更多',
      showingCount: '当前显示 {visible} 条匹配记录，已加载 {loaded} 条',
      showDetails: '详情',
      hideDetails: '收起',
      executionId: '执行 ID',
      executionIdPlaceholder: '按执行 ID 筛选',
      commandSearch: '命令搜索',
      commandSearchPlaceholder: '搜索命令文本',
      exactDate: '指定日期',
      recentDays: '最近天数',
      decision: '审批结果',
      allDecisions: '全部结果',
      semanticKind: '语义类别',
      allSemanticKinds: '全部语义类别',
      semanticCode: '语义代码',
      suggestedRules: '建议持久化规则',
      persistedRules: '已持久化规则',
      summary: {
        persistedRules: '已持久化规则 {count} 条',
        suggestedRules: '建议规则 {count} 条',
        noExtraDetails: '没有附加规则明细'
      },
      decisions: {
        allow: '已允许',
        deny: '已拒绝',
        allowForever: '永久允许'
      }
    },
    imageAttachments: {
      title: '图片附件',
      mode: '处理方式',
      modeDesc: '选择发送消息时如何处理图片附件。自动模式会优先使用当前支持视觉的模型；如果模型不支持视觉，则回退到本地 OCR。',
      auto: '自动（推荐）',
      localOcr: '始终使用本地 OCR',
      modelVision: '上传到模型（视觉）',
      allowUpload: '允许上传图片到模型',
      allowUploadDesc: '开启后，图片可能会被发送到所选 AI 服务用于视觉理解。',
      uploadDisabledWarning: '当前未允许上传图片到模型，请切回自动/本地 OCR 或开启上传权限。'
    },
    fileUploads: {
      title: '工作区管理',
      desc: '统一管理 Agent 工作区（含上传文件与执行产物）的存储配额与清理策略。',
      maxFileMb: '单文件大小上限 (MB)',
      maxTotalMb: '总存储上限 (MB)',
      maxPerConversation: '单会话文件上限',
      autoCleanup: '启用自动清理',
      retentionDays: '保留天数',
      clearAll: '清空全部文件',
      clearAllConfirm: '确认清空全部历史上传文件？',
      clearDate: '清空 {date}',
      clearDateConfirm: '确认清空 {date} 目录下的上传文件？',
      dateFilter: '日期筛选',
      conversationFilter: '会话筛选',
      conversationPlaceholder: '输入 conversation id',
      allDates: '全部日期',
      empty: '暂无历史上传文件。',
      totalFiles: '共 {count} 个文件',
      clearedCount: '已清理 {count} 个文件'
    },
    exploitdb: {
      title: 'ExploitDB 同步',
      desc: '配置并同步本地 ExploitDB 仓库，用于 PoC 检索。',
      repoUrl: '仓库地址',
      repoPath: '本地路径',
      selectPath: '选择路径',
      save: '保存',
      syncNow: '立即同步',
      refreshStatus: '刷新状态',
      repoReady: '仓库就绪',
      indexReady: '索引就绪',
      indexedEntries: '索引条目数',
      lastCommit: '最新提交',
      lastSync: '上次同步',
      indexedAt: '索引时间',
      syncSuccess: 'ExploitDB 同步完成'
    },
    subagent: {
      title: '子 Agent 设置',
      timeout: '任务超时时间',
      timeoutDesc: '等待子 Agent 任务完成的最大时间（60-7200 秒）',
      seconds: '秒'
    },
    completionGuard: {
      title: '完成防火墙',
      desc: '在任务标记成功前，校验最终回复质量、超时上下文以及必需产物证据，防止“假完成”。',
      enabled: '启用完成防火墙',
      enabledDesc: '开启后，会拦截可疑的短回复/未完成回复，并触发重试或显式失败。',
      enforceArtifactProof: '要求产物证据',
      enforceArtifactProofDesc: '当任务文本包含明确输出文件要求时，必须有产物存在证据才允许完成。',
      toolHeavyMinToolCalls: '工具密集阈值（调用次数）',
      minResponseCharsToolHeavy: '工具密集场景最小回复长度',
      minResponseCharsAfterTimeout: '超时/失败后最小回复长度',
      unfinishedPrefixMaxChars: '未完成前缀判定最大长度'
    }
  }
}
