...
pub fn graph_to_definition(graph: &WorkflowGraph) -> WorkflowDefinition {
    ...
    for e in &graph.edges {
        depends_map
            ...
            .or_default()
            .push(e.from_node.clone());
    }
    let steps: Vec<WorkflowStep> = graph
        .nodes
        .iter()
        .map(|n| WorkflowStep {
            id: n.id.clone(),
...
fn convert_core_to_rag(core: CoreRagConfig) -> RagConfig {
    RagConfig {
        ...
        chunking_strategy: match core.chunking_strategy {
            sentinel_db::core::models::rag_config::ChunkingStrategy::FixedSize => {
                sentinel_rag::config::ChunkingStrategy::FixedSize
            }
            sentinel_db::core::models::rag_config::ChunkingStrategy::RecursiveCharacter => {
                sentinel_rag::config::ChunkingStrategy::RecursiveCharacter
            }
            sentinel_db::core::models::rag_config::ChunkingStrategy::Semantic => {
                sentinel_rag::config::ChunkingStrategy::Semantic
            }
            sentinel_db::core::models::rag_config::ChunkingStrategy::StructureAware => {
...
pub async fn execute_workflow_steps(
    ...
    if let Err(e) = db_clone
        .create_workflow_run(
            ...
            &def_clone.metadata.id,
            &def_clone.metadata.name,
            &def_clone.metadata.version,
            "running",
            Utc::now(),
        )
        .await
...
pub async fn start_workflow_run(
    ...
    sentinel_license::ensure_feature_access(sentinel_license::LicensedFeature::WorkflowExecution)?;
...
pub async fn list_workflow_runs_paginated(
    ...
    let (runs, total) = db
        .list_workflow_runs_paginated(page, page_size, search.as_deref(), workflow_id.as_deref())
        .await
        .map_err(|e| e.to_string())?;
    Ok(serde_json::json!({
        "data": runs,
...
pub async fn get_workflow_run_detail(
    ...
    db.get_workflow_run_detail(&run_id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn save_workflow_definition(
    ...
    if is_tool_flag {
        ...
        let input_schema =
            ...
            .await;
        let output_schema =
            sentinel_tools::workflow_adapter::WorkflowToolAdapter::extract_output_schema(
                &workflow_json,
            );
        ...
        let executor =
            sentinel_tools::workflow_adapter::create_workflow_executor(workflow_id.clone());
        ...
        tracing::info!(
            "Refreshed workflow tool registration: {} ({})",
            graph.name,
            graph.id
        );
    }
...
pub async fn list_workflow_definitions(
    ...
    db.list_workflow_definitions(is_template.unwrap_or(false))
        .await
        .map_err(|e: anyhow::Error| e.to_string())
}
...
pub async fn delete_workflow_definition(
    ...
    db.delete_workflow_definition(&id)
        .await
        .map_err(|e: anyhow::Error| e.to_string())
}
...
pub async fn validate_workflow_graph(
    graph: WorkflowGraph,
) -> Result<Vec<WorkflowValidationIssue>, String> {
    ...
    // 检查必需输入端口
    for node in &graph.nodes {
        let required_inputs: Vec<&PortDef> =
            node.input_ports.iter().filter(|p| p.required).collect();
        let connected_ports: std::collections::HashSet<String> = graph
            .edges
            .iter()
...
use crate::scheduler::{ScheduleConfig, ScheduleExecutor, ScheduleInfo, WorkflowScheduler};
...
pub async fn start_workflow_schedule(
    ...
    let result = scheduler
        .start_schedule(workflow_id.clone(), workflow_name.clone(), config)
        .await;

    match &result {
        Ok(_) => tracing::info!(
            "[Scheduler] Schedule started successfully for: {}",
            workflow_name
        ),
        Err(e) => tracing::error!(
            "[Scheduler] Failed to start schedule for {}: {}",
            workflow_name,
            e
        ),
    }
