...
use crate::error_classifier::{ErrorCategory, ErrorClassifier, ErrorMatchRule, RecoveryStrategy};
...
impl From<ConfigRecoveryStrategy> for RecoveryStrategy {
    fn from(config_strategy: ConfigRecoveryStrategy) -> Self {
        match config_strategy {
            ...
            ConfigRecoveryStrategy::ExponentialBackoff {
                initial_delay_ms,
                max_delay_ms,
                multiplier,
...
impl ErrorConfigLoader {
    ...
    pub fn create_classifier_from_config(config: &ErrorConfig) -> Result<ErrorClassifier> {
        ...
        for (tool_name, tool_config) in &config.tool_specific {
            for config_rule in &tool_config.rules {
                match ErrorMatchRule::try_from(config_rule.clone()) {
                    Ok(rule) => {
                        debug!(
                            "Added tool-specific rule '{}' for tool '{}'",
                            rule.name, tool_name
                        );
                        rules.push(rule);
                    }
                    Err(e) => {
                        warn!(
                            "Failed to parse tool-specific rule '{}' for tool '{}': {}",
        ...
        info!(
            "Loaded {} error classification rules from configuration",
            rules.len()
        );
        ...
        ErrorConfig {
            ...
            rules: vec![
                ConfigRule {
                    ...
                    message_pattern: Some(
                        r"(?i)(target page|context|browser).*closed|page\.goto.*closed".to_string(),
                    ),
                    error_type: None,
                ...
                ConfigRule {
                    ...
                    error_code: None,
                    message_pattern: Some(
                        r"(?i)transport\s+(closed|error|failed)|connection\s+(closed|lost|failed)"
                            .to_string(),
                    ),
                    error_type: None,
                ...
                ConfigRule {
                    ...
                    error_code: None,
                    message_pattern: Some(
                        r"(?i)serde\s+error|serialization|deserialization|json\s+parse".to_string(),
                    ),
                    error_type: None,
                    category: "Serialization".to_string(),
                    recovery_strategy: ConfigRecoveryStrategy::Simple(
                        "ImmediateReconnect".to_string(),
                    ),
                },
            ],
