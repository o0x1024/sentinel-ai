...
impl BatchProgressManager {
    ...
    async fn execute_batch(&self, request: BatchRequest) -> Result<BatchResponse> {
        ...
        let responses = if request.parallel {
            self.execute_parallel(request.requests, request.max_concurrency, batch_id)
                .await?
        } else {
    ...
    async fn execute_single_request(&self, request: BatchRequestItem) -> Result<serde_json::Value> {
        match request {
            BatchRequestItem::CallTool {
                ...
                Ok(
                    serde_json::json!({ "tool": name, "result": "success", "output": format!("Tool {} executed with arguments: {}", name, arguments) }),
                )
            }
            BatchRequestItem::ListTools { .. } => {
                Ok(serde_json::json!({ "tools": [], "next_cursor": null }))
            }
            BatchRequestItem::GetResource { uri, .. } => {
                Ok(serde_json::json!({ "uri": uri, "content": "Resource content" }))
            }
        }
    }
    ...
    async fn update_batch_progress(
        ...
        if let Some(info) = self.batch_executions.write().await.get_mut(&batch_id) {
            ...
            info.progress =
                ((info.completed_items as f32 / info.total_items as f32) * 100.0) as u32;
            info.current_message = message;
        }
    }
    ...
    pub async fn cancel_batch(&self, batch_id: Uuid) -> Result<()> {
        if self.batch_executions.read().await.get(&batch_id).is_some() {
            self.update_batch_status(
                ...
                Some("Batch cancelled by user".to_string()),
            )
            .await;
...
pub fn create_default_batch_manager() -> BatchProgressManager {
    BatchProgressManager::new(100, 300, 10)
}
