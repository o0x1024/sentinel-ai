...
async fn import_from_docker_if_needed(image_path: &str) -> Result<Option<String>, OcrError> {
    ...
    let name = format!(
        "{:02x}{:02x}{:02x}{:02x}.{}",
        digest[0], digest[1], digest[2], digest[3], ext
    );
...
impl OcrTool {
    ...
    async fn download_model(model_name: &str, url: &str) -> Result<PathBuf, OcrError> {
        let path =
            Self::get_model_path(model_name).map_err(|e| OcrError::ModelError(e.to_string()))?;

        // If file exists, check if it's valid
        if path.exists() {
            let metadata = fs::metadata(&path).map_err(|e| {
                OcrError::ModelError(format!("Failed to check model metadata: {}", e))
            })?;
            ...
            if metadata.len() > min_size {
                ...
                tracing::warn!(
                    "File {} is too small ({} bytes, minimum: {}), re-downloading...",
                    model_name,
                    metadata.len(),
                    min_size
                );
        ...
        while let Some(item) = stream.next().await {
            let chunk =
                item.map_err(|e| OcrError::ModelError(format!("Error during download: {}", e)))?;
        ...
        let metadata = fs::metadata(&path).map_err(|e| {
            OcrError::ModelError(format!("Failed to check downloaded model metadata: {}", e))
        })?;
    ...
    fn run_ocr(
        ...
        let ocr = OAROCRBuilder::new(
            detection_path
                .to_str()
                .ok_or_else(|| OcrError::ModelError("Invalid detection path".to_string()))?,
            recognition_path
                .to_str()
                .ok_or_else(|| OcrError::ModelError("Invalid recognition path".to_string()))?,
            dict_path
                .to_str()
                .ok_or_else(|| OcrError::ModelError("Invalid dict path".to_string()))?,
        )
        .build()
...
impl Tool for OcrTool {
    ...
    async fn definition(&self, _prompt: String) -> rig::completion::ToolDefinition {
        rig::completion::ToolDefinition {
            ...
            parameters: serde_json::to_value(schemars::schema_for!(OcrArgs)).unwrap_or_default(),
        }
    }
