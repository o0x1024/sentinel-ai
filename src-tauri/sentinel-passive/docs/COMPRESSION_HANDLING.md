...
sentinel-passive 现在完全支持 HTTP 响应体的压缩格式检测和自动解压，支持以下压缩算法：
...
```bash
RUST_LOG=sentinel_passive=debug
```

示例输出：

```
DEBUG sentinel_passive::proxy: Detected content encoding: Some("gzip"), attempting decompression
DEBUG sentinel_passive::proxy: Decompressed gzip body: 1024 -> 8192 bytes
DEBUG sentinel_passive::proxy: Captured response body: compressed=1024 bytes, decompressed=8192 bytes
```
