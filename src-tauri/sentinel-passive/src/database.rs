...
use crate::{Finding, PassiveError, PluginMetadata, Result};
...
impl PassiveDatabaseService {
    ...
    pub async fn new(database_url: &str) -> Result<Self> {
        ...
        let db_path = database_url
            .strip_prefix("sqlite://")
            .ok_or_else(|| PassiveError::Database("Invalid database URL".to_string()))?;
        
        // 确保数据库目录存在
        if let Some(parent) = std::path::Path::new(db_path).parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| PassiveError::Database(format!("Failed to create database directory: {}", e)))?;
        }
        ...
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to connect: {}", e)))?;
    ...
    pub async fn run_migrations(&self) -> Result<()> {
        ...
        sqlx::query(
            r#"
            CREATE TABLE IF NOT EXISTS passive_vulnerabilities (
                id TEXT PRIMARY KEY,
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to create passive_vulnerabilities table: {}", e)))?;

        // 证据表
        sqlx::query(
            r#"
            CREATE TABLE IF NOT EXISTS passive_evidence (
                id TEXT PRIMARY KEY,
                ...
                FOREIGN KEY (vuln_id) REFERENCES passive_vulnerabilities(id) ON DELETE CASCADE
            )
            "#,
        )
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to create passive_evidence table: {}", e)))?;

        // 去重索引表
        sqlx::query(
            r#"
            CREATE TABLE IF NOT EXISTS passive_dedupe_index (
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to create passive_dedupe_index table: {}", e)))?;
        ...
        let indices = vec![
            ...
            "CREATE INDEX IF NOT EXISTS idx_passive_vulns_severity ON passive_vulnerabilities(severity)",
            "CREATE INDEX IF NOT EXISTS idx_passive_vulns_status ON passive_vulnerabilities(status)",
            "CREATE INDEX IF NOT EXISTS idx_passive_vulns_created ON passive_vulnerabilities(created_at DESC)",
            "CREATE INDEX IF NOT EXISTS idx_passive_evidence_vuln ON passive_evidence(vuln_id)",
            "CREATE INDEX IF NOT EXISTS idx_passive_evidence_timestamp ON passive_evidence(timestamp DESC)",
        ...
        for index_sql in indices {
            sqlx::query(index_sql)
                .execute(&self.pool)
                .await
                .map_err(|e| PassiveError::Database(format!("Failed to create index: {}", e)))?;
        }
        
        info!("Passive scan database schema created successfully");
        Ok(())
    }
    ...
    pub async fn insert_vulnerability(&self, finding: &Finding) -> Result<()> {
        ...
        sqlx::query(
            r#"
            INSERT INTO passive_vulnerabilities (
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to insert vulnerability: {}", e)))?;

        // 插入去重索引
        sqlx::query(
            r#"
            INSERT INTO passive_dedupe_index (signature, vuln_id) VALUES (?, ?)
            "#,
        )
        .bind(&signature)
        .bind(&finding.id)
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to insert dedupe index: {}", e)))?;
    ...
    pub async fn update_vulnerability_hit(&self, signature: &str) -> Result<()> {
        sqlx::query(
            r#"
            UPDATE passive_vulnerabilities 
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to update hit count: {}", e)))?;
    ...
    pub async fn check_signature_exists(&self, signature: &str) -> Result<bool> {
        let count: i64 = sqlx::query_scalar(
            r#"
            SELECT COUNT(*) FROM passive_dedupe_index WHERE signature = ?
            "#,
        )
        .bind(signature)
        .fetch_one(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to check signature: {}", e)))?;
    ...
    pub async fn list_vulnerabilities(
        ...
        let records = sqlx::query_as::<_, VulnerabilityRecord>(&query)
            .fetch_all(&self.pool)
            .await
            .map_err(|e| PassiveError::Database(format!("Failed to list vulnerabilities: {}", e)))?;
    ...
    pub async fn count_vulnerabilities(&self, filters: VulnerabilityFilters) -> Result<i64> {
        ...
        let row: (i64,) = sqlx::query_as(&query)
            .fetch_one(&self.pool)
            .await
            .map_err(|e| PassiveError::Database(format!("Failed to count vulnerabilities: {}", e)))?;
    ...
    pub async fn get_vulnerability_by_id(&self, vuln_id: &str) -> Result<Option<VulnerabilityRecord>> {
        ...
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to fetch vulnerability: {}", e)))?;
    ...
    pub async fn get_evidence_by_vuln_id(&self, vuln_id: &str) -> Result<Vec<EvidenceRecord>> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to fetch evidence: {}", e)))?;
    ...
    pub async fn update_vulnerability_status(&self, vuln_id: &str, status: &str) -> Result<()> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to update vulnerability status: {}", e)))?;
    ...
    pub async fn insert_evidence(&self, evidence: &EvidenceRecord) -> Result<()> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to insert evidence: {}", e)))?;
    ...
    pub async fn get_plugin_code(&self, plugin_id: &str) -> Result<Option<String>> {
        ...
        .fetch_optional(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to get plugin code: {}", e)))?;
    ...
    pub async fn get_plugin_by_id(&self, plugin_id: &str) -> Result<Option<serde_json::Value>> {
        ...
        .fetch_optional(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to get plugin: {}", e)))?;
    ...
    pub async fn update_plugin_enabled(&self, plugin_id: &str, enabled: bool) -> Result<()> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to update plugin status: {}", e)))?;
    ...
    pub async fn delete_plugin(&self, plugin_id: &str) -> Result<()> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to delete plugin: {}", e)))?;
    ...
    pub async fn insert_proxy_request(&self, request: &ProxyRequestRecord) -> Result<i64> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to insert proxy request: {}", e)))?;
    ...
    pub async fn list_proxy_requests(
        ...
        let records = sqlx::query_as::<_, ProxyRequestRecord>(&query)
            .fetch_all(&self.pool)
            .await
            .map_err(|e| PassiveError::Database(format!("Failed to list proxy requests: {}", e)))?;
    ...
    pub async fn count_proxy_requests(&self, filters: ProxyRequestFilters) -> Result<i64> {
        ...
        let row: (i64,) = sqlx::query_as(&query)
            .fetch_one(&self.pool)
            .await
            .map_err(|e| PassiveError::Database(format!("Failed to count proxy requests: {}", e)))?;
    ...
    pub async fn get_proxy_request_by_id(&self, id: i64) -> Result<Option<ProxyRequestRecord>> {
        ...
        .fetch_optional(&self.pool)
        .await
        .map_err(|e| PassiveError::Database(format!("Failed to fetch proxy request: {}", e)))?;
    ...
    pub async fn delete_proxy_requests_before(&self, before: DateTime<Utc>) -> Result<u64> {
        let result = sqlx::query("DELETE FROM proxy_requests WHERE timestamp < ?")
            ...
            .execute(&self.pool)
            .await
            .map_err(|e| PassiveError::Database(format!("Failed to delete old proxy requests: {}", e)))?;
