        ...
        let parameters = ToolParameters {
            parameters: vec![
                ParameterDefinition {
                    ...
                    description: "Optional strftime format, e.g. %Y-%m-%d %H:%M:%S".to_string(),
                    required: false,
                    default_value: None,
                },
                ParameterDefinition {
                    ...
                    description: "Optional unix timestamp in milliseconds to format; default now".to_string(),
                    required: false,
                    default_value: None,
                },
                ParameterDefinition {
                    ...
                    param_type: ParameterType::String,
                    description: "Optional timezone: 'local' (default) or 'utc'".to_string(),
                    required: false,
                    default_value: Some(json!("local")),
                },
            ],
        ...
        let output_json = match (tz, ts_ms) {
            ...
            ("local", Some(ms)) | (_, Some(ms)) => {
                ...
                json!({
                    ...
                    "formatted": dt_local.format(fmt).to_string(),
                    "epoch_ms": dt_local.timestamp_millis(),
                })
            }
            _ => {
