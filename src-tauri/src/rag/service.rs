...
use crate::rag::database::LanceDbManager;
...
impl RagService {
    ...
    pub async fn query_for_assistant(&self, request: &crate::rag::models::AssistantRagRequest) -> Result<(String, Vec<crate::rag::models::Citation>)> {
        ...
        println!("request.query,: {:?}", request.query,);
        // 获取集合名称
        let collection_name = if let Some(cid) = &request.collection_id {
            if cid.is_empty() {
                "default".to_string()
        ...
        // 使用 Rig + LanceDB 进行语义搜索
        let results: Vec<crate::rag::models::QueryResult> = match self.vector_store.search_similar(&collection_name, &request.query, top_k).await {
            Ok(results) => results,
            Err(e) => {
                warn!("向量搜索失败: {}, 返回空结果", e);
...
mod tests {
    ...
    async fn test_rag_service_creation() {
        ...
        let config = RagConfig {augmentation_enabled:false,database_path:Some(_temp_dir.path().to_path_buf()),chunk_size_chars:512,chunk_overlap_chars:50,top_k:5,mmr_lambda:0.7,batch_size:10,max_concurrent:4,embedding_provider:"rig".to_string(),embedding_model:"nomic-embed-text".to_string(),embedding_dimensions:None,embedding_api_key:None,embedding_base_url:Some("http://localhost:11434".to_string()),reranking_provider:None,reranking_model:None,reranking_enabled:false,similarity_threshold:0.7,context_window_size:1, chunking_strategy: todo!(), min_chunk_size_chars: todo!(), max_chunk_size_chars: todo!() };
