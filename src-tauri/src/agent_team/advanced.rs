...
pub fn get_builtin_scenarios() -> Vec<ScenarioArtifactSuite> {
    vec![
        ...
        ScenarioArtifactSuite {
            ...
            display_name: "产品研发",
            artifacts: vec![
                ScenarioArtifact { artifact_type: "prd", title: "产品需求文档（PRD）", depends_on: vec![] },
                ScenarioArtifact { artifact_type: "architecture", title: "架构设计文档", depends_on: vec!["prd"] },
                ScenarioArtifact { artifact_type: "detailed_design", title: "详细设计文档", depends_on: vec!["architecture"] },
                ScenarioArtifact { artifact_type: "test_plan", title: "测试计划", depends_on: vec!["prd", "architecture"] },
                ScenarioArtifact { artifact_type: "workflow_tasks", title: "工作流任务拆解", depends_on: vec!["prd"] },
            ],
        },
        // 2. 代码审计
        ScenarioArtifactSuite {
            ...
            display_name: "代码审计",
            artifacts: vec![
                ScenarioArtifact { artifact_type: "vulnerability_report", title: "漏洞分析报告", depends_on: vec![] },
                ScenarioArtifact { artifact_type: "audit_summary", title: "审计执行摘要", depends_on: vec!["vulnerability_report"] },
                ScenarioArtifact { artifact_type: "remediation_plan", title: "修复优先级计划", depends_on: vec!["vulnerability_report"] },
            ],
        },
        // 3. 安全事件响应
        ScenarioArtifactSuite {
            ...
            display_name: "安全事件响应",
            artifacts: vec![
                ScenarioArtifact { artifact_type: "incident_review", title: "安全事件复盘报告", depends_on: vec![] },
                ScenarioArtifact { artifact_type: "timeline_analysis", title: "攻击链时序分析", depends_on: vec!["incident_review"] },
                ScenarioArtifact { artifact_type: "hardening_checklist", title: "加固清单", depends_on: vec!["incident_review"] },
            ],
        },
        // 4. 红蓝对抗复盘
        ScenarioArtifactSuite {
            ...
            display_name: "红蓝对抗复盘",
            artifacts: vec![
                ScenarioArtifact { artifact_type: "red_team_report", title: "红队攻击报告", depends_on: vec![] },
                ScenarioArtifact { artifact_type: "blue_team_response", title: "蓝队响应报告", depends_on: vec!["red_team_report"] },
                ScenarioArtifact { artifact_type: "lessons_learned", title: "经验总结与改进计划", depends_on: vec!["red_team_report", "blue_team_response"] },
            ],
        },
        // 5. 运维变更
        ScenarioArtifactSuite {
            ...
            display_name: "运维变更",
            artifacts: vec![
                ScenarioArtifact { artifact_type: "change_plan", title: "变更计划书", depends_on: vec![] },
                ScenarioArtifact { artifact_type: "rollback_plan", title: "回滚预案", depends_on: vec!["change_plan"] },
                ScenarioArtifact { artifact_type: "capacity_assessment", title: "容量评估报告", depends_on: vec![] },
            ],
        },
    ]
}
...
pub fn build_scenario_artifact_prompt(
    ...
    let scenario_instructions = match artifact_type {
        ...
        "workflow_tasks" => "将任务拆解为 JSON 格式，包含 id、name、assignee、priority、depends_on 字段。",
...
pub fn compute_version_diff(old: &str, new: &str) -> String {
    ...
    let added: Vec<&str> = new_lines.iter().filter(|l| !old_lines.contains(l)).copied().collect();
    let removed: Vec<&str> = old_lines.iter().filter(|l| !new_lines.contains(l)).copied().collect();
    ...
    let mut parts = vec![];
    if !added.is_empty() {
        let preview: String = added.iter().take(3).map(|l| format!("+ {}", l)).collect::<Vec<_>>().join("\n");
        parts.push(format!("**新增** ({} 行):\n{}", added.len(), preview));
    }
    if !removed.is_empty() {
        let preview: String = removed.iter().take(3).map(|l| format!("- {}", l)).collect::<Vec<_>>().join("\n");
...
pub fn parse_judge_review(content: &str) -> JudgeReview {
    ...
    // 判断 verdict
    let verdict = if lower.contains("建议修订") || lower.contains("request_revision") || lower.contains("需重新讨论") {
...
impl PromptInjectionGuard {
    ...
    pub fn sanitize(input: &str, context: InjectionContext) -> SanitizedInput {
        ...
        // 3. 检测跨角色数据泄露意图
        if matches!(context, InjectionContext::BlackboardEntry | InjectionContext::HumanMessage) {
            let exfil_patterns = ["send to", "report to", "leak", "exfiltrate", "外泄", "泄露给"];
        ...
        let is_safe = !issues.iter().any(|i| i.severity == IssueSeverity::Critical || i.severity == IssueSeverity::High);
...
impl TeamSessionFinOps {
    ...
    pub fn record_llm_call(
        ...
        let stats = self.per_member.entry(member_id.to_string()).or_insert_with(|| MemberTokenStats {
    ...
    pub fn generate_report(&self) -> String {
        let mut lines = vec![
            format!("## FinOps 报告 — 会话 {}", &self.session_id[..8.min(self.session_id.len())]),
            format!(""),
            format!("**总 Token**: {} | **估算成本**: ${:.4} USD", self.total_tokens, self.estimated_cost_usd),
            format!(""),
            format!("### 按角色消耗"),
        ];

        let mut members: Vec<&MemberTokenStats> = self.per_member.values().collect();
        members.sort_by(|a, b| (b.input_tokens + b.output_tokens).cmp(&(a.input_tokens + a.output_tokens)));

        for m in members {
            let total = m.input_tokens + m.output_tokens;
            let pct = if self.total_tokens > 0 { total * 100 / self.total_tokens } else { 0 };
            lines.push(format!(
...
mod tests {
    ...
    fn test_builtin_scenarios() {
        ...
        let test_plan = product.artifacts.iter().find(|a| a.artifact_type == "test_plan").unwrap();
