...
impl SecurityTestManager {
    ...
    pub async fn update_step(
        ...
        session_id: &str,
        step_id: &str,
        update_fn: impl FnOnce(&mut TestStep),
    ) -> Result<()> {
        let mut sessions = self.sessions.write().await;
        let session = sessions
            .get_mut(session_id)
    ...
    pub async fn get_session_stats(&self, session_id: &str) -> Result<SessionStats> {
        ...
        let critical_findings = session
            .findings
            .iter()
            .filter(|f| f.risk_level == RiskImpact::Critical)
            .count();
        let high_findings = session
            .findings
            .iter()
            .filter(|f| f.risk_level == RiskImpact::High)
            .count();
        let medium_findings = session
            .findings
            .iter()
            .filter(|f| f.risk_level == RiskImpact::Medium)
            .count();
