...
pub trait Database: Send + Sync + std::fmt::Debug {
    ...
    async fn set_config(&self, category: &str, key: &str, value: &str, description: Option<&str>) -> Result<()>;
...
impl DatabaseService {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        self.pool = Some(pool);
        tracing::info!("Database initialization completed: {}", self.db_path.display());
    ...
    async fn create_database_schema(&self, pool: &SqlitePool) -> Result<()> {
        ...
        // 创建索引
        // 赏金项目索引
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_bounty_projects_platform ON bounty_projects(platform)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_bounty_projects_status ON bounty_projects(status)").execute(&mut *tx).await?;
        ...
        // 扫描任务索引
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_scan_tasks_project_id ON scan_tasks(project_id)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_scan_tasks_status ON scan_tasks(status)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_scan_tasks_created_at ON scan_tasks(created_at DESC)").execute(&mut *tx).await?;

        // 资产索引
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_assets_project_id ON assets(project_id)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_assets_type ON assets(asset_type)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_assets_risk_level ON assets(risk_level)").execute(&mut *tx).await?;

        // 漏洞索引
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_vulnerabilities_project_id ON vulnerabilities(project_id)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_vulnerabilities_severity ON vulnerabilities(severity)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_vulnerabilities_status ON vulnerabilities(status)").execute(&mut *tx).await?;
        ...
        // 提交记录索引
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_submissions_project_id ON submissions(project_id)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_submissions_status ON submissions(status)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_submissions_submitted_at ON submissions(submitted_at DESC)").execute(&mut *tx).await?;

        // MCP工具索引
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_mcp_tools_category ON mcp_tools(category)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_mcp_tools_status ON mcp_tools(status)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_mcp_tools_name ON mcp_tools(name)").execute(&mut *tx).await?;

        // 工具执行记录索引
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_tool_executions_tool_id ON tool_executions(tool_id)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_tool_executions_status ON tool_executions(status)").execute(&mut *tx).await?;
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_ai_messages_timestamp ON ai_messages(timestamp DESC)").execute(&mut *tx).await?;

        // 收益索引
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_earnings_project_id ON earnings(project_id)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_earnings_payment_status ON earnings(payment_status)").execute(&mut *tx).await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_earnings_created_at ON earnings(created_at DESC)").execute(&mut *tx).await?;

        // 配置索引
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_configurations_category ON configurations(category)").execute(&mut *tx).await?;

        // 创建MCP服务器配置索引
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_mcp_server_configs_name ON mcp_server_configs(name)").execute(&mut *tx).await?;
    ...
    pub fn get_pool(&self) -> Result<&SqlitePool> {
        self.pool.as_ref().ok_or_else(|| anyhow::anyhow!("数据库未初始化"))
    }
            ...
            for (i, column) in row.columns().iter().enumerate() {
                ...
                let value: Value = match column.type_info().name() {
                    ...
                    "REAL" => {
                        ...
                        val.map(|v| Value::Number(serde_json::Number::from_f64(v).unwrap_or_else(|| 0.into()))).unwrap_or(Value::Null)
                    }
                    _ => Value::Null,
                };
    ...
    pub async fn get_project(&self, id: &str) -> Result<BountyProject> {
        ...
        let row = sqlx::query_as::<_, BountyProject>(
            "SELECT * FROM bounty_projects WHERE id = ?"
        )
        .bind(id)
        .fetch_one(pool)
        .await?;
    ...
    pub async fn get_scan_task(&self, id: &str) -> Result<Option<ScanTask>> {
        ...
        let row = sqlx::query_as::<_, ScanTask>(
    ...
    pub async fn get_conversation(&self, id: &str) -> Result<Option<AiConversation>> {
        ...
        let row = sqlx::query_as::<_, AiConversation>(
    ...
    /// 根据项目ID获取漏洞
    pub async fn get_vulnerabilities_by_project(&self, project_id: &str) -> Result<Vec<Vulnerability>> {
    ...
    pub async fn update_mcp_server_config_enabled(
        &self,
        id: &str,
        enabled: bool,
    ) -> Result<()> {
        let pool = self.get_pool()?;
        sqlx::query(
            "UPDATE mcp_server_configs SET is_enabled = ? WHERE id = ?",
        )
        .bind(enabled)
        .bind(id)
        .execute(pool)
    ...
    pub async fn get_mcp_server_config_by_name(&self, name: &str) -> Result<Option<McpServerConfig>> {
        let pool = self.get_pool()?;
    ...
    pub async fn update_mcp_server_config(
        ...
        // 如果找不到现有配置，使用默认值
        let url = existing.as_ref().map(|c| c.url.clone()).unwrap_or_else(|| "http://localhost:8080".to_string());
        let connection_type = existing.as_ref().map(|c| c.connection_type.clone()).unwrap_or_else(|| "stdio".to_string());
    ...
    async fn delete_ai_role(&self, role_id: &str) -> Result<()> {
        ...
        tracing::info!("Delete operation affected {} rows for role ID: {}", rows_affected, role_id);
        
        if rows_affected == 0 {
            tracing::warn!("No rows were affected when deleting role ID: {}. Role might not exist.", role_id);
        }
    ...
    async fn insert_default_data(&self, pool: &SqlitePool) -> Result<()> {
        ...
        let default_configs = vec![
            ("cfg_ai_default_model", "ai", "default_model", "\"gpt-4\"", "AI默认模型"),
            ("cfg_ai_max_tokens", "ai", "max_tokens", "4000", "AI最大令牌数"),
            ("cfg_ai_temperature", "ai", "temperature", "0.7", "AI温度参数"),
            ("cfg_scan_default_timeout", "scan", "default_timeout", "3600", "扫描默认超时时间（秒）"),
            ("cfg_scan_max_concurrent", "scan", "max_concurrent_tasks", "5", "最大并发扫描任务数"),
            ("cfg_mcp_auto_connect", "mcp", "auto_connect_servers", "true", "是否自动连接MCP服务器"),
            ("cfg_notification_enabled", "notification", "enabled", "true", "是否启用通知"),
            ("cfg_general_theme", "general", "theme", "\"dark\"", "界面主题"),
        ];
        ...
        let default_tools = vec![
            (
                "tool_subfinder", "subfinder", "Subfinder", "快速子域名发现工具", "2.6.3", 
                "reconnaissance", "builtin",
            ...
            (
                "tool_nmap", "nmap", "Nmap", "网络发现和安全审计工具", "7.94",
                "reconnaissance", "builtin",
            ...
            (
                "tool_nuclei", "nuclei", "Nuclei", "基于模板的漏洞扫描器", "3.1.0",
                "vulnerability_scanning", "builtin",
            ...
            (
                "tool_httpx", "httpx", "Httpx", "HTTP工具包", "1.3.7",
                "reconnaissance", "builtin",
        ...
        for (id, name, display_name, description, version, category, tool_type, config_schema, default_config, capabilities, supported_platforms) in default_tools {
            sqlx::query(r#"
        ...
        let (id, name, description, url, connection_type, auth_type, auth_config, command, args, is_enabled, is_default, auto_connect, retry_count, retry_delay, timeout, metadata) = default_mcp_server;
...
impl Database for DatabaseService {
    ...
    async fn get_ai_conversations(&self) -> Result<Vec<AiConversation>> {
        let pool = self.get_pool()?;
        let conversations = sqlx::query_as::<_, AiConversation>("SELECT * FROM ai_conversations ORDER BY updated_at DESC")
            .fetch_all(pool)
            .await?;
        Ok(conversations)
    }
    ...
    async fn get_ai_messages_by_conversation(&self, conversation_id: &str) -> Result<Vec<AiMessage>> {
        let pool = self.get_pool()?;
        let messages = sqlx::query_as::<_, AiMessage>("SELECT * FROM ai_messages WHERE conversation_id = ? ORDER BY timestamp ASC")
            .bind(conversation_id)
            .fetch_all(pool)
            .await?;
        Ok(messages)
    ...
    async fn get_configs_by_category(&self, category: &str) -> Result<Vec<Configuration>> {
        let pool = self.get_pool()?;
        let configs = sqlx::query_as::<_, Configuration>("SELECT * FROM configurations WHERE category = ?")
            .bind(category)
            .fetch_all(pool)
            .await?;
        Ok(configs)
    ...
    async fn get_config(&self, category: &str, key: &str) -> Result<Option<String>> {
        let pool = self.get_pool()?;
        let result: Option<(String,)> = sqlx::query_as::<_, (String,)>("SELECT value FROM configurations WHERE category = ? AND key = ?")
            .bind(category)
            .bind(key)
            .fetch_optional(pool)
            .await?;
