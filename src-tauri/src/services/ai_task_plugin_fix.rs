...
use crate::commands::ai_task_support::{
    ...
    merge_external_profile_prompt, resolve_external_llm_config, run_external_text_task,
...
pub struct PluginFixTaskRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginFixTaskResult {
    pub run_id: String,
...
pub async fn run_plugin_fix_task(
    runtime: &Arc<SystemAgentRuntime>,
    ai_manager: &Arc<AiServiceManager>,
    request: PluginFixTaskRequest,
) -> Result<PluginFixTaskResult> {
    let payload = serde_json::to_value(&request)?;
    let tracked_run = start_external_profile_run(runtime, PLUGIN_FIX_PROFILE_ID, payload).await;
    ...
    match execute_plugin_fix(ai_manager, runtime, &request, &run_id).await {
        Ok(result) => {
            ...
            complete_external_profile_run_success(runtime, &tracked_run, output).await;
            Ok(result)
        }
        Err(error) => {
            complete_external_profile_run_failure(runtime, &tracked_run, error.to_string()).await;
            Err(error)
        }
    }
}
...
async fn execute_plugin_fix(
    ...
    let prompt = merge_external_profile_prompt(
    ...
    if fixed_code.trim().is_empty() {
        return Err(anyhow!("Plugin fix task returned empty code"));
    }
