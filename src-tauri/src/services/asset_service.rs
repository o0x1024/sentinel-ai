...
impl AssetService {
    ...
    pub async fn create_asset(
        ...
        self.db
            .create_asset(request, created_by)
            .await
    ...
    pub async fn list_assets(
        ...
        self.db
            .list_assets(filter, limit, offset)
            .await
            .map_err(|e: anyhow::Error| format!("Database error: {}", e))
    }
    ...
    pub async fn import_assets(
        ...
        self.db
            .import_assets(request, created_by)
            .await
            .map_err(|e: anyhow::Error| format!("Database error: {}", e))
    }
    ...
    pub async fn extract_assets_from_scan(
        ...
        if let Some(port_scan_results) = scan_results.get("port_scan") {
            if let Some(open_ports) = port_scan_results
                .get("open_ports")
                    ...
                    if let (Some(ip), Some(port)) = (
                        ...
                        let ip_request = CreateAssetRequest {
                            ...
                            metadata: Some({
                                ...
                                meta.insert(
                                    "scan_type".to_string(),
                                    serde_json::Value::String("port_scan".to_string()),
                                );
                                meta
                            }),
                        ...
                        if self
                            .db
                            .find_asset_by_type_and_value(&AssetType::Ip, ip)
                            .await
                            .map_err(|e: anyhow::Error| format!("Database error: {}", e))?
                            .is_none()
                        {
                            let ip_asset = self
                                .db
                                .create_asset(ip_request, created_by.clone())
                                .await
                                .map_err(|e: anyhow::Error| format!("Database error: {}", e))?;
                            ...
                            let port_request = CreateAssetRequest {
                                ...
                                metadata: Some({
                                    ...
                                    if let Some(service) =
                                        port_info.get("service").and_then(|v| v.as_str())
                                    {
                                        meta.insert(
                                            "service".to_string(),
                            ...
                            if self
                                .db
                                .find_asset_by_type_and_value(&AssetType::Port, &port_value)
                                .await
                                .map_err(|e: anyhow::Error| format!("Database error: {}", e))?
                                .is_none()
                            {
                                let port_asset = self
                                    .db
                                    .create_asset(port_request, created_by.clone())
                                    .await
                                    .map_err(|e: anyhow::Error| format!("Database error: {}", e))?;
        ...
        if let Some(domain_scan_results) = scan_results.get("domain_scan") {
            if let Some(domains) = domain_scan_results
                .get("domains")
                .and_then(|v| v.as_array())
            {
                    ...
                    if let Some(domain) = domain_info.get("domain").and_then(|v| v.as_str()) {
                        let domain_request = CreateAssetRequest {
                            ...
                            metadata: Some({
                                ...
                                meta.insert(
                                    "scan_type".to_string(),
                                    serde_json::Value::String("domain_scan".to_string()),
                                );
                        ...
                        if self
                            .db
                            .find_asset_by_type_and_value(&domain_request.asset_type, domain)
                            .await
                            .map_err(|e: anyhow::Error| format!("Database error: {}", e))?
                            .is_none()
                        {
                            let domain_asset = self
                                .db
                                .create_asset(domain_request, created_by.clone())
                                .await
                                .map_err(|e: anyhow::Error| format!("Database error: {}", e))?;
                    ...
                    if let Some(url) = website_info.get("url").and_then(|v| v.as_str()) {
                        let website_request = CreateAssetRequest {
                            ...
                            metadata: Some({
                                ...
                                meta.insert(
                                    "scan_type".to_string(),
                                    serde_json::Value::String("web_scan".to_string()),
                                );
                                if let Some(status) =
                                    website_info.get("status").and_then(|v| v.as_u64())
                                {
                                    meta.insert(
                                ...
                                if let Some(title) =
                                    website_info.get("title").and_then(|v| v.as_str())
                                {
                                    meta.insert(
                                        "title".to_string(),
                        ...
                        if self
                            .db
                            .find_asset_by_type_and_value(&AssetType::Website, url)
                            .await
                            .map_err(|e: anyhow::Error| format!("Database error: {}", e))?
                            .is_none()
                        {
                            let website_asset = self
                                .db
                                .create_asset(website_request, created_by.clone())
                                .await
                                .map_err(|e: anyhow::Error| format!("Database error: {}", e))?;
    ...
    fn assess_port_risk(&self, port: u64) -> RiskLevel {
        match port {
            // 高风险端口
            21 | 23 | 135 | 139 | 445 | 1433 | 1521 | 3306 | 3389 | 5432 | 5900 | 6379 => {
                RiskLevel::High
            }
            // 中等风险端口
    ...
    pub async fn get_related_assets(&self, asset_id: &str) -> Result<Vec<Asset>, String> {
        let (incoming, outgoing) = self
            .db
            .get_asset_relationships(asset_id)
            .await
            .map_err(|e: anyhow::Error| format!("Database error: {}", e))?;
