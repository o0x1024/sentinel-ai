...
impl QualityModel {
    ...
    pub fn train(&mut self) -> Result<TrainingReport> {
        ...
        let mean_quality: f32 =
            self.samples.iter().map(|s| s.actual_score).sum::<f32>() / self.samples.len() as f32;
    ...
    pub fn extract_features(code: &str) -> CodeFeatures {
        ...
        let payload_count = code.matches("payload").count() + code.matches("test").count();
