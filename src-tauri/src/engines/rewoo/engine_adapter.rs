...
use crate::engines::rewoo::{
    rewoo_planner::ReWOOPlanner, rewoo_solver::ReWOOSolver, rewoo_types::*,
...
impl ReWooEngine {
    ...
    pub async fn new_with_dependencies(
        ...
        let framework_adapter =
            crate::tools::get_framework_adapter(crate::tools::FrameworkType::ReWOO).await?;
        ...
        let engine_info = EngineInfo {
            ...
            description:
                "Reasoning without Observation - Efficient task execution with upfront planning"
                    .to_string(),
    ...
    pub async fn execute(&mut self, task: &AgentTask) -> Result<AgentExecutionResult> {
        ...
        info!(
            "ReWOO: conversation_id={:?}, message_id={:?}, execution_id={:?}",
        ...
        // 创建标准消息发送器
        let emitter = if let Some(app) = &self.app_handle {
            Some(StandardMessageEmitter::new(
                Arc::new(app.clone()),
                execution_id.clone(),
                message_id.clone().unwrap_or_else(|| execution_id.clone()),
        ...
        // 发送Planning阶段开始消息
        if let Some(ref emitter) = emitter {
            emitter.emit_thinking("开始生成执行计划...");
        }

        let available_tools = self.worker.get_available_tools().await;
        let plan = self
            .planner
            .plan(&task.description, &available_tools, None, &execution_id)
            .await?;

        info!(
            "ReWOO Engine: Generated plan with {} steps",
            plan.steps.len()
        );
        ...
        for (idx, rewoo_step) in plan.steps.iter().enumerate() {
            ...
            // 发送工具执行开始消息
            if let Some(ref emitter) = emitter {
                emitter.emit_step_update(idx, &plan_step.tool, "executing");
            }
                ...
                .await
            {
                Ok(result) => {
                    if result.success {
                        ...
                        state
                            .results
                            .insert(plan_step.variable.clone(), result_value.clone());

                        // 发送工具执行结果
                        if let Some(ref emitter) = emitter {
                            emitter.emit_tool_result(&plan_step.tool, &result_value);
                        }

                        info!(
                            "ReWOO Engine: Step {} completed successfully",
                            plan_step.variable
                        );
                    } else {
                        let error_msg = format!("Tool execution failed: {:?}", result.error);
                        warn!(
                            "ReWOO Engine: Step {} failed: {}",
                            plan_step.variable, error_msg
                        );
        ...
        // 发送求解阶段开始消息
        if let Some(ref emitter) = emitter {
            emitter.emit_thinking("开始生成最终答案...");
        }
        ...
        let plan_steps_json: Vec<serde_json::Value> = plan
            .steps
            .iter()
            .map(|step| {
                serde_json::json!({
        ...
        // 保存完整的AI响应消息到数据库
        if let (Some(conv_id), Some(msg_id)) = (conversation_id.as_ref(), message_id.as_ref()) {
            ...
            let structured_data = serde_json::json!({
                "rewooData": {
                    ...
                    "results": tool_results.iter().map(|(k, v)| {
                        serde_json::json!({
                            "variable": k,
                            "value": v,
                        })
                    }).collect::<Vec<_>>(),
                    "finalAnswer": final_answer.clone(),
                }
            });
            ...
            let ai_msg = AiMessage {
                ...
                metadata: None,
                token_count: Some(final_answer.len() as i32),
                cost: None,
                tool_calls: None,
                attachments: None,
                timestamp: Utc::now(),
                architecture_type: Some("ReWOO".to_string()),
            ...
            if let Err(e) = self.db_service.upsert_ai_message_append(&ai_msg).await {
                warn!("Failed to save ReWOO message to database: {}", e);
            } else {
                info!("ReWOO message saved to database: {}", msg_id);
            }
        }
...
impl ExecutionEngine for ReWooEngine {
    ...
    async fn create_plan(&self, task: &AgentTask) -> anyhow::Result<ExecutionPlan> {
        ...
        let plan = self
            .planner
            .plan(&task.description, &available_tools, None, &task.id)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to create ReWOO plan: {}", e))?;
    ...
    async fn execute_plan(&self, plan: &ExecutionPlan) -> anyhow::Result<AgentExecutionResult> {
        ...
        let final_answer = self
            ...
            .solve(
                ...
                &tool_results,
                &task.id,
                None, // execute_plan方法没有emitter上下文
            )
            .await?;
