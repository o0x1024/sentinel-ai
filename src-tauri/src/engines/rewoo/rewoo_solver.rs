...
impl ReWOOSolver {
    ...
    async fn call_llm(&self, system_prompt: &str, user_prompt: &str, execution_id: &str) -> Result<String> {
            ...
            .await 
        {
            ...
            Ok(None) | Err(_) => {
                // 回退到默认服务
                warn!("ReWOO Solver: Scheduler config not available, using fallback model: {}", self.config.model_name);
        ...
        info!("ReWOO Solver: Using provider={}, model={}, execution_id={}", config.provider, config.model, execution_id);
        ...
        info!("ReWOO Solver: Generated answer with {} chars", content.len());
        Ok(content)
    }
}