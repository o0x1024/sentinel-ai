...
impl ElementManager {
    ...
    fn generate_fingerprint(element: &AnnotatedElement, page_url: &str) -> String {
        ...
        // 使用 selector + type + text(前20字符) 生成指纹
        element.selector.hash(&mut hasher);
        element.element_type.hash(&mut hasher);
        element.text.chars().take(20).collect::<String>().hash(&mut hasher);
    ...
    fn is_hover_candidate(element: &AnnotatedElement) -> bool {
        ...
        if element.attributes.get("aria-expanded").is_some() {
            return true;
        }
        ...
        if let Some(class) = element.attributes.get("class") {
            ...
            if class_lower.contains("dropdown") 
                || class_lower.contains("menu") 
                || class_lower.contains("nav")
                || class_lower.contains("submenu")
                || class_lower.contains("popover")
                || class_lower.contains("tooltip") {
                return true;
            }
        }
    ...
    pub fn update_page_elements(&mut self, elements: &[AnnotatedElement], page_url: &str) -> usize {
        ...
        for element in elements {
            ...
            // 保存当前页面映射
            self.current_page_mapping.insert(element.index, fingerprint.clone());
            ...
            if !self.all_elements.contains_key(&fingerprint) {
                ...
                self.all_elements.insert(fingerprint, info);
            }
        }

        if new_count > 0 {
            info!("Discovered {} new elements on page: {}", new_count, page_url);
        }
    ...
    pub fn add_dynamic_component(&mut self, component: DynamicComponent) {
        // 检查是否已存在
        let exists = self.dynamic_components.iter()
            .any(|c| c.selector == component.selector && c.component_type == component.component_type);
        
        if !exists {
            info!("Discovered dynamic component: {} ({})", component.component_type, component.selector);
    ...
    pub fn get_unexplored_components(&self) -> Vec<&DynamicComponent> {
        self.dynamic_components.iter().filter(|c| !c.explored).collect()
    }
    ...
    pub fn component_coverage_percentage(&self) -> f32 {
        ...
        let explored = self.dynamic_components.iter().filter(|c| c.explored).count();
...
mod tests {
    ...
    fn make_element(index: u32, element_type: &str, text: &str, attributes: HashMap<String, String>) -> AnnotatedElement {
        AnnotatedElement {
            index,
            ...
            bounding_box: BoundingBox { x: 0.0, y: 0.0, width: 100.0, height: 50.0 },
            attributes,
        }
    }
