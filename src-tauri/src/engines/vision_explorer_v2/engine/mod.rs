...
use crate::engines::vision_explorer_v2::agent_framework::Agent;
...
use crate::engines::vision_explorer_v2::core::{Event, PageContext};
        ...
        while let Some(event) = event_rx.recv().await {
            ...
            match &event {
                ...
                Event::TaskAssigned { agent_id, .. } => {
                    ...
                    if agent_id == &navigator.metadata().id {
                    ...
                    } else if agent_id == &visual_agent.metadata().id {
                    ...
                    } else if agent_id == &operator.metadata().id {
                    ...
                    } else if agent_id == &auth_agent.metadata().id {
                            ...
                            if let Some(data) = &result.data {
                                    ...
                                    >(data.clone())
                                {
                                    emitter.emit_analysis(
                                        step_count,
                                        &perception.content_summary,
                                        &vec![], // suggested_actions are now generated by Planner
                                        0.0,
                                    );
                                }
                            }
            ...
            match planner.decide_next_step().await {
                Ok(next_tasks) => {
                    ...
                    } // End for next_task in next_tasks
                }
                Err(e) => {
