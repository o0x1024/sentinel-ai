        ...
        .set_task_executor(move |task| {
            ...
            Box::pin(async move {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        tracing::warn!("Failed to fetch existing assets for task targets: {}", e)
                    }
                }
                ...
                for plugin in plugins_to_run {
                    ...
                    if input.get("targets").is_none() {
                        ...
                        input["targets"] =
                            serde_json::to_value(&targets).unwrap_or(serde_json::json!([]));
                    }
                    ...
                    if let Some(output) = &result.output {
                        ...
                        for sub_str in discovered_assets {
                            ...
                            // Check existence
                            let exists = db
                                .get_bounty_asset_by_canonical_url(&task.program_id, clean_domain)
                                .await
                                .map(|opt| opt.is_some())
                                .unwrap_or(false);
...
pub async fn monitor_trigger_task(
    ...
    if targets.is_empty() {
        tracing::warn!(
            "No targets found for program {}, aborting manual execution",
            task.program_id
        );
