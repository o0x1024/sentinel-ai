...
/// 本地拦截动作（用于命令参数）
pub type InterceptAction = TrafficInterceptAction;
...
pub async fn upsert_agent_audit_findings(
    ...
    for finding in request.findings {
        let mut files = finding.files.unwrap_or_default();
        let title = finding
            .title
            .clone()
        ...
        let mut description = finding.description.unwrap_or_default();
    ...
    Ok(CommandResponse::ok(UpsertAgentAuditFindingsResult {
        inserted,
        updated_hits,
...
pub async fn start_proxy_listener(
    ...
    // 调用启动代理的命令
    match start_traffic_analysis(app, state, Some(config)).await {
        Ok(response) => {
            if response.success {
...
pub async fn stop_proxy_listener(
    ...
    // 调用停止代理的命令
    match stop_traffic_analysis(app, state).await {
        Ok(response) => {
            if response.success {
...
pub async fn drop_intercepted_websocket(
    ...
    if let Some(msg) = messages.remove(&id) {
        if msg
            .response_tx
            .send(sentinel_traffic::InterceptAction::Drop)
            .is_err()
        {
