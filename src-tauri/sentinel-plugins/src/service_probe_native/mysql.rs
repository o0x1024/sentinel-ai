    ...
    let mut stream = match connect_result {
        ...
        Ok(Err(error)) => {
            return ProbeEvidence {
                ...
                port: target.port,
                protocol,
                probe_name: "mysql_greeting".to_string(),
                success: false,
                available: false,
                banner: None,
                server_header: None,
        ...
        Err(_) => {
            return ProbeEvidence {
                ...
                headers: BTreeMap::new(),
                status_code: None,
                error: Some("MySQL connect timed out".to_string()),
            };
        }
    };

    if let Some(report) = stage_reporter {
        report(2, 4, "Reading MySQL greeting");
    }
