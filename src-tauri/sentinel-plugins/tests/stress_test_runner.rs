...
impl TestSuiteReport {
    ...
    pub fn generate_recommendations(&mut self) {
        ...
        // 错误率建议
        let high_error_tests: Vec<_> = self
            .results
            .iter()
            .filter(|r| {
        ...
        if !high_error_tests.is_empty() {
            self.recommendations.push(format!(
                "⚠️  {} test(s) have error rate > 10%. Review error messages for details.",
                high_error_tests.len()
            ));
        }

        // 并发建议
        let concurrency_tests: Vec<_> = self
            .results
            .iter()
            .filter(|r| matches!(r.category, TestCategory::Concurrency))
            .collect();

        if let Some(best_concurrency) = concurrency_tests.iter().max_by(|a, b| {
            a.throughput_per_sec
                .partial_cmp(&b.throughput_per_sec)
                .unwrap()
        }) {
            self.recommendations.push(format!(
                "✓ Optimal concurrency configuration: {} (throughput: {:.2} ops/sec)",
        ...
        for category in &[
            ...
            for result in category_results {
                ...
                let error_rate = if result.success_count + result.error_count > 0 {
                    result.error_count as f64 / (result.success_count + result.error_count) as f64
                        * 100.0
                } else {
                    0.0
                };
    ...
    pub fn print_console_report(&self) {
        ...
        println!(
            "  {:<40} {:<10} {:<15} {:<15}",
            "Test Name", "Status", "Duration", "Throughput"
        );
        println!("  {}", "-".repeat(80));
