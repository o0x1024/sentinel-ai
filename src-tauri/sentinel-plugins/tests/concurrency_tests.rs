...
async fn test_sustained_high_concurrency() {
    ...
    println!(
        ...
        total_success as f64 / total_duration.as_secs_f64()
    );
    println!(
        "  Error Rate: {:.2}%",
        total_errors as f64 / (total_success + total_errors) as f64 * 100.0
    );
    println!("{}", "=".repeat(80));
}
...
async fn test_plugin_executor_concurrency() {
    ...
    println!(
        "Error Rate: {:.2}%",
        error_count as f64 / iterations as f64 * 100.0
    );
}
...
async fn test_plugin_manager_multi_plugin_concurrency() {
    ...
    for i in 0..num_plugins {
        ...
        manager
            ...
            .unwrap();
        manager
            .set_plugin_code(format!("concurrency-test-{}", i), code)
            .await
            .unwrap();
    }
...
async fn test_race_condition() {
    ...
    println!(
        "Shared Data Length: {} (expected: {})",
        data_len, iterations
    );
    println!(
        "Counter Match: {}",
        if final_counter == iterations {
            "✓"
        } else {
            "✗"
        }
    ...
    println!(
        "Data Match: {}",
        if data_len == iterations { "✓" } else { "✗" }
    );

    assert_eq!(
        final_counter, iterations,
        "Race condition detected in counter"
    );
    assert_eq!(
        data_len, iterations,
        "Race condition detected in shared data"
    );
}
