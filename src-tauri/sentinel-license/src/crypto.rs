...
impl std::str::FromStr for LicenseKey {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let json_str = String::from_utf8(decoded).map_err(|_| CryptoError::InvalidKeyFormat)?;

        serde_json::from_str(&json_str).map_err(|_| CryptoError::InvalidKeyFormat)
    }
}
...
fn get_embedded_public_key() -> Result<VerifyingKey, CryptoError> {
    ...
    VerifyingKey::from_bytes(&key_array).map_err(|_| CryptoError::InvalidKeyFormat)
}
