...
impl PacketCaptureService {
    ...
    pub(crate) fn parse_packet(id: u64, data: &[u8], iface_name: &str) -> Option<CapturedPacket> {
        let ethernet = EthernetPacket::new(data)?;
        let timestamp = chrono::Utc::now().timestamp_millis();
        let mut layers = Vec::new();

        // Frame layer
        let frame_display = format!(
            "Frame {}: {} bytes on wire, {} bytes captured on interface {}",
            id,
            data.len() * 8,
            data.len(),
            iface_name
        ...
        layers.push(ProtocolLayer {
            ...
            fields: vec![
                ...
                ProtocolField::new(
                    "Frame Length",
                    &format!("{} bytes ({} bits)", data.len(), data.len() * 8),
                ),
        ...
        layers.push(ProtocolLayer {
            ...
            fields: vec![
                ...
                ProtocolField::new(
                    "Type",
                    &format!("{:?} (0x{:04x})", ether_type, ether_type_val),
                ),
            ],
        });
        ...
        if let Some(ipv4) = Ipv4Packet::new(data) {
            ...
            let flags_children = vec![
                ProtocolField::new(
                    &format!(
                        "{}.. .... = Reserved bit",
                        if flags_val & 0b100 != 0 { "1" } else { "0" }
                    ),
                    "Not set",
                ),
                ProtocolField::new(
                    &format!(".{}. .... = Don't fragment", if df { "1" } else { "0" }),
                    if df { "Set" } else { "Not set" },
                ),
                ProtocolField::new(
                    &format!("..{} .... = More fragments", if mf { "1" } else { "0" }),
                    if mf { "Set" } else { "Not set" },
                ),
            ];
            ...
            layers.push(ProtocolLayer {
                ...
                fields: vec![
                    ProtocolField::new(
                        &format!("{:04b} .... = Version", version),
                        &version.to_string(),
                    ),
                    ProtocolField::new(
                        &format!(".... {:04b} = Header Length", ihl),
                        &format!("{} bytes ({})", ihl * 4, ihl),
                    ),
                    ProtocolField::with_children(
                        "Differentiated Services Field",
                        &format!(
                            "0x{:02x} (DSCP: 0x{:02x}, ECN: 0x{:02x})",
                            (dscp << 2) | ecn,
                            dscp,
                            ecn
                        ),
                    ...
                    ProtocolField::with_children(
                        ...
                        &format!(
                            "{}{}",
                            if df { ", Don't fragment" } else { "" },
                            if mf { ", More fragments" } else { "" }
                        ),
                        flags_children,
                    ),
        ...
        if let Some(ipv6) = Ipv6Packet::new(data) {
            ...
            layers.push(ProtocolLayer {
                ...
                fields: vec![
                    ...
                    ProtocolField::new(
                        "Next Header",
                        &format!("{:?} ({})", next_header, next_header.0),
                    ),
        ...
        if let Some(tcp) = TcpPacket::new(data) {
            ...
            let flags_children = vec![
                ProtocolField::new(
                    ...
                    if cwr { "Set" } else { "Not set" },
                ),
                ProtocolField::new(
                    &format!(".{}..... = ECN-Echo", if ece { "1" } else { "0" }),
                    if ece { "Set" } else { "Not set" },
                ),
                ProtocolField::new(
                    &format!("..{}.... = Urgent", if urg { "1" } else { "0" }),
                    if urg { "Set" } else { "Not set" },
                ),
                ProtocolField::new(
                    &format!(
                        "...{}... = Acknowledgment",
                        if ack_flag { "1" } else { "0" }
                    ),
                    if ack_flag { "Set" } else { "Not set" },
                ),
                ProtocolField::new(
                    &format!("....{}.. = Push", if psh { "1" } else { "0" }),
                    if psh { "Set" } else { "Not set" },
                ),
                ProtocolField::new(
                    &format!(".....{}. = Reset", if rst { "1" } else { "0" }),
                    if rst { "Set" } else { "Not set" },
                ),
                ProtocolField::new(
                    &format!("......{} = Syn", if syn { "1" } else { "0" }),
                    if syn { "Set" } else { "Not set" },
                ),
                ProtocolField::new(
                    &format!(".......{} = Fin", if fin { "1" } else { "0" }),
                    if fin { "Set" } else { "Not set" },
                ),
            ];
            ...
            layers.push(ProtocolLayer {
                ...
                fields: vec![
                    ...
                    ProtocolField::new(
                        &format!("{:04b} .... = Header Length", data_offset),
                        &format!("{} bytes ({})", data_offset * 4, data_offset),
                    ),
                    ProtocolField::with_children(
        ...
        if let Some(udp) = UdpPacket::new(data) {
            ...
            let (protocol, info) = if src_port == 53 || dst_port == 53 {
                if let Some((dns_info, dns_layer)) = parse_dns_content(payload, src_port == 53) {
                    ...
                    (
                        "DNS".to_string(),
                        format!("{} → {} Len={}", src_port, dst_port, length),
                    )
                }
            } else {
    ...
    fn parse_icmpv6(
        ...
        if let Some(icmpv6) = Icmpv6Packet::new(data) {
            ...
            let info = format!(
                "{} (type={}, code={})",
                type_name, icmpv6_type.0, icmpv6_code.0
            );
            (
                src_ip.to_string(),
                dst_ip.to_string(),
...
pub use crate::packet_capture_file_extractor::{ExtractedFile, FileExtractor};
pub use crate::packet_capture_pcap_ops::PcapFileOps;
...
fn detect_app_protocol_tcp(src_port: u16, dst_port: u16) -> String {
    let port = if is_well_known_port(dst_port) {
        dst_port
    } else {
        src_port
    };
...
fn is_well_known_port(port: u16) -> bool {
    port < 1024
        || matches!(
            port,
            3306 | 5432 | 6379 | 27017 | 3389 | 8080 | 8443 | 3000 | 8000 | 5353 | 5355
        )
}
