                ...
                while let Some(content) = stream.next().await {
                    match content {
                        ...
                        Ok(StreamedAssistantContent::ReasoningDelta { reasoning, id }) => {
                            // DeepSeek and some OpenAI-compatible providers stream reasoning as deltas.
                            // Keep these deltas in accumulated_reasoning so the next tool-call turn can
                            // replay assistant(tool_calls + reasoning) correctly.
                            if let Some(ref mut existing) = accumulated_reasoning {
                                if existing.id.is_none() && id.is_some() {
                                    existing.id = id.clone();
                                }
                                existing.reasoning.push(reasoning.clone());
                            } else {
                                accumulated_reasoning = Some(rig::message::Reasoning {
