    ...
    fn try_from(message: message::Message) -> Result<Self, Self::Error> {
        match message {
            ...
            message::Message::Assistant { content, .. } => {
                ...
                // Avoid emitting an empty assistant shell message. DeepSeek may reject
                // assistant history entries with missing reasoning_content in tool-call flows.
                if !text_content.is_empty() || reasoning_content.is_some() {
...
impl TryFrom<(&str, CompletionRequest)> for DeepseekCompletionRequest {
    ...
    fn try_from((model, req): (&str, CompletionRequest)) -> Result<Self, Self::Error> {
        ...
        // DeepSeek reasoner requires `reasoning_content` to be present on assistant
        // messages that contain tool calls. If the provider stream did not emit
        // reasoning (or it was unavailable), we still include an empty field to satisfy
        // request validation.
        for message in &mut full_history {
            if let Message::Assistant {
                tool_calls,
                reasoning_content,
                ..
