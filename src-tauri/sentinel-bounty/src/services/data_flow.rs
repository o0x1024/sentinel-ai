...
use crate::services::workflow_artifact::{
    ArtifactExtractor, ArtifactType, LiveHostsArtifact, SubdomainsArtifact,
};
...
impl PluginPortRegistry {
    ...
    fn register_builtin_plugins(&mut self) {
        ...
        // SSRF Detector
        self.output_specs.insert(
            "ssrf_detector".to_string(),
            vec![("findings".to_string(), ArtifactType::Finding)],
        );
        self.input_specs.insert(
            "ssrf_detector".to_string(),
            vec![(
        ...
        self.output_specs.insert(
            "cors_misconfiguration".to_string(),
            vec![("findings".to_string(), ArtifactType::Finding)],
        );
        self.input_specs.insert(
            "cors_misconfiguration".to_string(),
        ...
        self.output_specs.insert(
            "open_redirect_detector".to_string(),
            vec![("findings".to_string(), ArtifactType::Finding)],
        );
        self.input_specs.insert(
            "open_redirect_detector".to_string(),
...
impl DataFlowResolver {
    ...
    pub fn subdomains_to_targets(artifact: &SubdomainsArtifact) -> serde_json::Value {
        let targets: Vec<String> = artifact
            .subdomains
            .iter()
...
mod tests {
    ...
    fn test_extract_by_path() {
        ...
        let result = resolver
            .extract_by_path(&data, "subdomains[*].subdomain")
            .unwrap();
    ...
    fn test_resolve_inputs() {
        ...
        let resolved =
            resolver.resolve_inputs("http_prober", &step_config, &[("step-1", upstream_output)]);
