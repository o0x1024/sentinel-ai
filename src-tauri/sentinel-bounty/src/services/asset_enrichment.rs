...
impl AssetEnrichmentService {
    ...
    pub async fn enrich_asset(&self, asset_id: &str) -> Result<()> {
        ...
        info!(
            "Enriching asset: {} ({})",
            asset.canonical_url, asset.asset_type
        );
    ...
    async fn resolve_domain_ips(domain: &str) -> Result<Vec<String>> {
        ...
        let ips: Vec<String> = addresses.map(|addr| addr.ip().to_string()).collect();
            ...
            match Self::fetch_url_info(&asset.canonical_url).await {
                Ok(info) => {
                    ...
                    if updated_asset.waf_detected.is_none() {
                        updated_asset.waf_detected =
                            Some(Self::detect_waf(&info.headers).to_string());
                        changed = true;
                    }

                    if updated_asset.cdn_detected.is_none() {
                        updated_asset.cdn_detected =
                            Self::detect_cdn(&info.headers).map(|s| s.to_string());
                        changed = true;
                    }

                    info!(
                        "Fetched HTTP info for {}: status={}",
                        asset.canonical_url, info.status_code
                    );
                }
                Err(e) => warn!(
                    "Failed to fetch URL info for {}: {}",
                    asset.canonical_url, e
                ),
            }
        }
    ...
    async fn fetch_url_info(_url: &str) -> Result<UrlInfo> {
        ...
        Err(anyhow::anyhow!(
            "URL fetching not yet implemented - requires HTTP client"
        ))
    }
