...
pub fn build_context_from_results(results: &[QueryResult]) -> String {
    let parts: Vec<String> = results.iter().map(|r| r.chunk.content.clone()).collect();
    parts.join("\n\n")
}
...
fn extract_signal_tokens(s: &str) -> Vec<String> {
    ...
    for p in &products { if s.contains(p) { tokens.push((*p).to_string()); } }
    for t in &vul_terms { if s.contains(t) { tokens.push((*t).to_string()); } }
    for t in &en_vul_terms { if lower.contains(t) { tokens.push((*t).to_string()); } }

    // Split words and keep informative ones
    let stop_words = [
...
fn assemble_fallback_query(tokens: &[String], max_terms: usize, max_len: usize) -> String {
    ...
    for t in tokens { if is_cve(t) || is_product(t) || is_vul(t) { prioritized.push(t.clone()); } }
...
fn assemble_query(tokens: &[String], max_terms: usize, max_len: usize) -> String {
    ...
    for t in tokens {
        ...
        out.push(t.clone());
        let s = out.join(" ");
        if s.len() > max_len { out.pop(); break; }
    }
    out.join(" ")
}
...
fn dedup_preserve_order(list: Vec<String>) -> Vec<String> {
    ...
    let mut out = Vec::new();
    for x in list.into_iter() { if seen.insert(x.clone()) { out.push(x); } }
    out
}
