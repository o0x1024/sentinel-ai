...
fn extract_signal_tokens(s: &str) -> Vec<String> {
    ...
    for m in port_re.find_iter(s) {
        tokens.push(m.as_str().to_string());
    }
    for m in url_like_re.find_iter(s) {
        tokens.push(m.as_str().to_string());
    }
    ...
    for w in s.split(|c: char| c.is_whitespace() || ",.;:()[]{}<>\"'|".contains(c)) {
        ...
        if stop_words.iter().any(|sw| sw.eq_ignore_ascii_case(w)) {
            continue;
        }
        if w.len() <= 1 {
            continue;
...
fn assemble_fallback_query(tokens: &[String], max_terms: usize, max_len: usize) -> String {
    ...
    for t in tokens {
        if is_cve(t) || is_product(t) || is_vul(t) {
            prioritized.push(t.clone());
        }
    }
    if prioritized.is_empty() {
        prioritized = tokens.to_vec();
    }
...
fn count_cjk(s: &str) -> usize {
    s.chars()
        .filter(|c| matches!(c, '\u{4E00}'..='\u{9FA5}' | '\u{3400}'..='\u{4DBF}'))
        .count()
}
