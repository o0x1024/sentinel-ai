...
impl<D: RagDatabase> RagService<D> {
    ...
    pub async fn new(config: RagConfig, database: Arc<D>) -> Result<Self> {
        ...
        if config.reranking_enabled {
            match (
                config.reranking_provider.clone(),
                ...
                (Some(provider), Some(model)) => {
                    match create_reranking_provider(
                        ...
                        Ok(provider_impl) => {
                            ...
                            reranker = Some(mgr);
                            info!(
                                "Reranking enabled with provider: {} and model: {}",
                                provider, model
                            );
                        }
                        Err(e) => {
    ...
    async fn ensure_default_collection(&self) -> Result<()> {
        ...
        match self
            .database
            .get_rag_collection_by_name(DEFAULT_COLLECTION_NAME)
            .await?
        {
            Some(_) => {
                info!("默认RAG集合已存在");
        ...
        self.vector_store
            .create_collection(DEFAULT_COLLECTION_NAME, 768)
            .await?;
        Ok(())
    }
    ...
    fn is_poc_document(&self, file_path: &str, content: &str) -> bool {
        ...
        let keyword_count = poc_keywords
            .iter()
    ...
    pub async fn ingest_source(&self, request: IngestRequest) -> Result<IngestResponse> {
        ...
        self.ingestion_status
            .write()
            .await
            .insert(task_id.clone(), ingestion_status.clone());
        ...
        let collection_id = if let Some(cid) = request.collection_id.or(Some("".to_string())) {
            ...
            match self
                .database
                .get_rag_collection_by_name(default_name)
                .await?
            {
                ...
                None => {
                    // 创建默认集合
                    self.database
                        .create_rag_collection(default_name, Some("默认集合"))
                        .await?
                }
            }
        };
        ...
        // 分块处理
        let (document_source, mut chunks) =
            match self.chunker.process_document(&request.file_path).await {
                ...
                Err(e) => {
                    ...
                    self.ingestion_status
                        .write()
                        .await
                        .insert(task_id.clone(), ingestion_status);
        ...
        if is_poc {
            ...
            for chunk in &mut chunks {
                chunk
                    .metadata
                    .custom_fields
                    .insert("document_type".to_string(), "poc".to_string());
                chunk
                    .metadata
                    .custom_fields
                    .insert("requires_completeness".to_string(), "true".to_string());
            }
        }
        ...
        let collection_name = if let Some(collection) = self
            .database
            .get_rag_collection_by_id(&collection_id)
            .await?
        {
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                if let Err(del_err) = self.database.delete_rag_document(&document_id).await {
                    ...
                    info!(
                        "Rolled back document {} due to vector insertion failure",
                        document_id
                    );
                }
        ...
        let processing_time = ingestion_status
            .completed_at
            .unwrap()
    ...
    pub async fn ingest_text(
        ...
        let collection_name = if let Some(collection) = self
            .database
            .get_rag_collection_by_id(&target_collection_id)
            .await?
        {
            collection.name
        } else {
        ...
        if let Err(e) = self
            .database
            .update_collection_stats(&target_collection_id)
            .await
        {
            warn!("更新集合统计失败: {}", e);
        }
    ...
    async fn expand_chunk_context(&self, results: Vec<QueryResult>) -> Result<Vec<QueryResult>> {
        ...
        info!(
            "Expanding chunk context: before={}, after={}",
            before, after
        );
        ...
        for result in results {
            ...
            if is_poc {
                ...
                info!(
                    "POC document detected, expanding context to before={}, after={}",
                    before, after
                );
            }
            ...
            expanded_chunk.content_hash =
                format!("{:x}", md5::compute(expanded_chunk.content.as_bytes()));
            ...
            expanded_chunk
                .metadata
                .custom_fields
                .insert("expanded".to_string(), "true".to_string());
            ...
            expanded_chunk
                .metadata
                .custom_fields
                .insert("skip_similarity_filter".to_string(), "true".to_string());
    ...
    pub async fn query(&self, request: RagQueryRequest) -> Result<RagQueryResponse> {
        ...
        // 构建上下文
        let context_parts: Vec<String> = query_results
            .iter()
    ...
    pub async fn query_for_assistant(
        &self,
        request: &crate::models::AssistantRagRequest,
    ) -> Result<(String, Vec<crate::models::Citation>)> {
        ...
        for collection_name in collection_names {
                ...
                .await
            {
                Ok(results) => {
                    info!(
                        "Collection '{}' returned {} results",
                        collection_name,
                        results.len()
                    );
                    for (i, r) in results.iter().enumerate() {
                        info!(
                            "  Result {}: score={:.4}, content_hash={}, preview={}",
                            i + 1,
                            r.score,
                            &r.chunk.content_hash[..8],
                            r.chunk.content.chars().take(50).collect::<String>()
                        );
                    }
        ...
        let mut results: Vec<crate::models::QueryResult> =
        ...
        results.retain(|r| {
            ...
            if skip_filter {
                info!(
                    "Skipping similarity filter for expanded chunk in assistant query: {}",
                    r.chunk.id
                );
                true
            } else {
        ...
        if should_rerank {
            ...
            if let Some(reranker) = &self.reranker {
                match reranker
                    .rerank(&request.query, &documents, Some(top_k))
                    .await
                {
                    Ok(reranked) => {
                    ...
                    Err(e) => {
                        warn!(
                            "Reranking failed: {}. Falling back to vector search order.",
                            e
                        );
                    }
                }
    ...
    pub async fn get_query_history(
        ...
        self.database
            .get_rag_query_history(Some(collection_id), limit)
            .await
    }
    ...
    pub async fn clear_collection(&self, collection_id: &str) -> Result<()> {
            ...
            .await?
        {
            ...
            self.vector_store
                .create_collection(&collection.name, 768)
                .await?;
        }
