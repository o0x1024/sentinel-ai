...
use crate::config::{ChunkingStrategy, RagConfig, SupportedFileType};
use crate::models::{ChunkMetadata, DocumentChunk, DocumentSource, IngestionStatusEnum};
...
impl DocumentChunker {
    ...
    pub async fn process_document(
        ...
        let content = self
            .extract_text_content(file_path, &supported_file_type)
            .await?;
    ...
    fn chunk_text(
        ...
        match self.config.chunking_strategy {
            ...
            ChunkingStrategy::StructureAware => {
                self.chunk_text_structure_aware(content, file_path, source)
            }
        }
    }
    ...
    fn chunk_text_fixed_size(
        ...
        while cursor < total_len {
            ...
            let chunk =
                self.create_chunk(&chunk_str, chunk_index, cursor, end_char, file_path, source)?;
            chunks.push(chunk);
            chunk_index += 1;
    ...
    fn recursive_split(
        ...
        while start < chars.len() {
            ...
            if !chunk_text.trim().is_empty() {
                let chunk =
                    self.create_chunk(&chunk_text, chunk_index, start, end, file_path, source)?;
                chunks.push(chunk);
                chunk_index += 1;
            }
    ...
    fn chunk_text_semantic(
        ...
        if !current_chunk.trim().is_empty()
            && current_chunk.chars().count() >= self.config.min_chunk_size_chars
        {
            let chunk = self.create_chunk(
                &current_chunk,
                chunk_index,
    ...
    fn chunk_text_structure_aware(
        ...
        match file_type {
            ...
            _ => {
                debug!("Structure-aware chunking not implemented, fallback");
                self.chunk_text_recursive(content, file_path, source)
            }
        }
    }
        ...
        for line in lines {
            if line.starts_with('#') {
                ...
                if current_section.chars().count() > self.config.max_chunk_size_chars {
                    ...
                    let mut sub_chunks =
                        self.chunk_text_recursive(&chunk_content, file_path, source)?;
...
impl TextCleaner {
    pub fn new() -> Self {
        Self { whitespace_regex: Regex::new(r"\s+").unwrap(), special_chars_regex: Regex::new(r"[^\w\s\u4e00-\u9fff.,;:()\x22\x27\-/=&%+#@\[\]{}!?！？。，；：（）【】《》\u201c\u201d\u2018\u2019—…]").unwrap() }
    }
