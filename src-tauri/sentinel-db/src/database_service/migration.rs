...
impl DatabaseMigration {
    ...
    pub async fn migrate(&self) -> Result<()> {
        ...
        for table in sorted_tables {
            ...
            self.import_table_to_pool(&table, &table_data, target)
                .await?;
        }
    ...
    async fn sort_tables_by_dependencies(&self, tables: Vec<String>) -> Result<Vec<String>> {
        ...
        let dependencies: std::collections::HashMap<&str, Vec<&str>> = [
            ...
            (
                "scan_tasks",
                vec!["scan_sessions", "scan_stages", "vulnerabilities"],
            ),
            ("scan_sessions", vec!["scan_stages"]),
            ("assets", vec!["asset_relationships"]),
            (
                "rag_collections",
                vec!["rag_document_sources", "rag_chunks", "rag_queries"],
            ),
            ("rag_document_sources", vec!["rag_chunks"]),
            (
                "traffic_vulnerabilities",
                vec!["traffic_evidence", "traffic_dedupe_index"],
            ),
        ...
        for (referenced, dependents) in &dependencies {
            for dependent in dependents {
                if tables.contains(&referenced.to_string())
                    && tables.contains(&dependent.to_string())
                {
                    graph
                        .get_mut(&referenced.to_string())
                        .unwrap()
    ...
    fn convert_to_postgresql_sql(&self, sqlite_sql: &[String]) -> Vec<String> {
        info!(
            "Converting {} SQLite statements to PostgreSQL",
            sqlite_sql.len()
        );
        sqlite_sql
            .iter()
            .map(|sql| {
                ...
                pg_sql =
                    pg_sql.replace("INTEGER PRIMARY KEY AUTOINCREMENT", "BIGSERIAL PRIMARY KEY");
    ...
    fn convert_to_mysql_sql(&self, sqlite_sql: &[String]) -> Vec<String> {
        ...
        sqlite_sql
            ...
            .map(|sql| {
                ...
                mysql_sql =
                    mysql_sql.replace("INTEGER NOT NULL DEFAULT 0", "BIGINT NOT NULL DEFAULT 0");
                ...
                mysql_sql =
                    mysql_sql.replace("CREATE TABLE IF NOT EXISTS", "CREATE TABLE IF NOT EXISTS");
                mysql_sql =
                    mysql_sql.replace("CREATE INDEX IF NOT EXISTS", "CREATE INDEX IF NOT EXISTS");
    ...
    async fn export_table(&self, table_name: &str) -> Result<TableData> {
        ...
        match &self.source_pool {
            DatabasePool::PostgreSQL(pool) => {
                ...
                let schema: Vec<ColumnInfo> = first_row
                    .columns()
                    .iter()
                    .map(|col| ColumnInfo {
            ...
            DatabasePool::SQLite(p) => {
                ...
                let schema: Vec<ColumnInfo> = columns_rows
                    .into_iter()
                    .map(|r| ColumnInfo {
                        name: r.get("name"),
    ...
    async fn import_table(&self, table_name: &str, table_data: &TableData) -> Result<()> {
        self.import_table_to_pool(table_name, table_data, &self.source_pool)
            .await
    }

    async fn import_table_to_pool(
        &self,
        table_name: &str,
        ...
        info!(
            "Importing {} rows into table {}",
            table_data.rows.len(),
            table_name
        );

        // Ensure table exists in target DB; some targets may be fresh and have no schema yet.
        self.ensure_target_table(table_name, &table_data.schema, pool)
            .await?;
        ...
        match pool {
                ...
                match sqlx::query(&truncate_query).execute(p).await {
                    ...
                    Err(e) => {
                        ...
                        sqlx::query(&delete_query)
                            ...
                            .await
                            .context(format!(
                                "Failed to clear PostgreSQL table {} before import",
                                table_name
                            ))?;
            ...
            DatabasePool::SQLite(p) => {
                ...
                sqlx::query(&delete_query)
                    ...
                    .await
                    .context(format!(
                        "Failed to clear SQLite table {} before import",
                        table_name
                    ))?;
            }
            DatabasePool::MySQL(p) => {
                ...
                sqlx::query(&delete_query)
                    ...
                    .await
                    .context(format!(
                        "Failed to clear MySQL table {} before import",
                        table_name
                    ))?;
            }
        }
                    ...
                    for col in &columns {
                        ...
                        query = match value {
                            Value::String(s) => {
                                ...
                                if col.to_lowercase().contains("time")
                                    || col.to_lowercase().contains("date")
                                    || col.to_lowercase() == "created_at"
                                    || col.to_lowercase() == "updated_at"
                                {
                                    ...
                                    match base64::engine::general_purpose::STANDARD.decode(s) {
                                        Ok(binary_data) => query.bind(binary_data),
                                        Err(_) => {
    ...
    #[allow(dead_code)]
    async fn import_table_selective(
        &self,
        table_name: &str,
        table_data: &TableData,
        pool: &DatabasePool,
        ...
        info!(
            "Performing selective import for table {} ({} rows)",
            table_name,
            table_data.rows.len()
        );
            ...
            match self.import_single_row(table_name, row_data, pool).await {
                ...
                Err(e) => {
                    if e.to_string().contains("foreign key constraint") {
                        skipped_count += 1;
                        warn!(
                            "Skipped row {} in table {} due to foreign key constraint: {}",
                            row_index + 1,
                            table_name,
                            e
                        );
        ...
        info!(
            "Selective import completed for {}: {} successful, {} skipped",
            table_name, success_count, skipped_count
        );
        Ok(())
    }
