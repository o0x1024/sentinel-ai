...
fn category_to_str(c: &PromptCategory) -> &'static str {
    match c {
...
fn str_to_category(s: &str) -> Option<PromptCategory> {
    match s {
...
fn template_type_to_str(t: &TemplateType) -> &'static str {
    match t {
...
fn str_to_template_type(s: &str) -> Option<TemplateType> {
    match s {
...
impl DatabaseService {
    pub async fn list_prompt_templates_internal(&self) -> Result<Vec<PromptTemplate>> {
        ...
        Ok(rows.into_iter().map(|row| self.row_to_prompt_template(row)).collect())
    }

    pub async fn get_prompt_template_internal(&self, id: i64) -> Result<Option<PromptTemplate>> {
        let pool = self.get_pool()?;
        let r = sqlx::query(
            r#"SELECT id, name, description, content, is_default, is_active, created_at, updated_at,
               category, template_type, is_system, priority, tags, variables, version 
        ...
        Ok(r.map(|row| self.row_to_prompt_template(row)))
    }

    pub async fn create_prompt_template_internal(&self, t: &PromptTemplate) -> Result<i64> {
        let pool = self.get_pool()?;
        let category_s = t.category.as_ref().map(category_to_str);
        let template_type_s = t.template_type.as_ref().map(template_type_to_str);
    ...
    pub async fn update_prompt_template_internal(&self, id: i64, t: &PromptTemplate) -> Result<()> {
        let pool = self.get_pool()?;
        let category_s = t.category.as_ref().map(category_to_str);
        let template_type_s = t.template_type.as_ref().map(template_type_to_str);
        let tags_json = serde_json::to_string(&t.tags).unwrap_or_else(|_| "[]".to_string());
    ...
    pub async fn delete_prompt_template_internal(&self, id: i64) -> Result<()> {
        let pool = self.get_pool()?;
        sqlx::query("DELETE FROM prompt_templates WHERE id = ?").bind(id).execute(pool).await?;
        Ok(())
    }

    pub async fn list_prompt_templates_filtered_internal(
        &self,
        ...
        if let Some(cat) = &category { query.push_str(&format!(" AND category = '{}'", category_to_str(cat))); }
        if let Some(tt) = &template_type { query.push_str(&format!(" AND template_type = '{}'", template_type_to_str(tt))); }
    ...
    pub async fn duplicate_prompt_template_internal(&self, id: i64, new_name: Option<String>) -> Result<i64> {
        if let Some(template) = self.get_prompt_template_internal(id).await? {
            let mut t = template;
            t.id = None;
            ...
            t.created_at = None;
            t.updated_at = None;
            self.create_prompt_template_internal(&t).await
        } else {
            Err(anyhow::anyhow!("Template not found"))
        }
    }

    pub async fn get_active_prompt_template_by_type_internal(&self, template_type: TemplateType) -> Result<Option<PromptTemplate>> {
        let templates = self.list_prompt_templates_filtered_internal(
            None,
            Some(template_type),
            None
        ).await?;
    ...
    pub async fn evaluate_prompt_internal(&self, id: i64, context: serde_json::Value) -> Result<String> {
        if let Some(template) = self.get_prompt_template_internal(id).await? {
            self.evaluate_prompt_content_internal(&template.content, context).await
        } else {
            Err(anyhow::anyhow!("Template not found"))
        }
    }

    pub async fn evaluate_prompt_content_internal(&self, content: &str, context: serde_json::Value) -> Result<String> {
    ...
    pub async fn insert_default_templates_internal(&self) -> Result<()> {
        // Implementation here if needed
        Ok(())
    }

    fn row_to_prompt_template(&self, row: sqlx::sqlite::SqliteRow) -> PromptTemplate {
        ...
        PromptTemplate {
            ...
            created_at,
            updated_at,
            category: category.and_then(|c| str_to_category(&c)),
            template_type: template_type.and_then(|t| str_to_template_type(&t)),
