...
impl HttpRequestTool {
    ...
    fn categorize_error(&self, error: &reqwest::Error) -> String {
        if error.is_timeout() {
            return "请求超时：服务器响应时间过长".to_string();
        }
        
        if error.is_connect() {
            return format!("连接失败：无法连接到目标服务器 ({})", 
                          error.url().map(|u| u.to_string()).unwrap_or_else(|| "未知地址".to_string()));
        }
        
        if error.is_request() {
            return "请求格式错误：请检查URL、请求头或请求体格式".to_string();
        }
        
        if error.is_redirect() {
            return "重定向错误：重定向次数过多或重定向循环".to_string();
        }
        
        if error.is_body() {
            return "响应体读取错误：服务器响应数据格式异常".to_string();
        }
        
        if error.is_decode() {
            return "响应解码错误：无法解析服务器响应内容".to_string();
        }
        
        // 检查是否是DNS解析错误
        let error_str = error.to_string();
        if error_str.contains("dns") || error_str.contains("resolve") {
            return "DNS解析失败：无法解析域名，请检查网络连接或域名是否正确".to_string();
        }
        
        // 检查是否是SSL/TLS错误
        if error_str.contains("ssl") || error_str.contains("tls") || error_str.contains("certificate") {
            return "SSL/TLS错误：证书验证失败，可尝试设置verify_ssl为false".to_string();
        }
        
        // 检查是否是网络不可达
        if error_str.contains("unreachable") || error_str.contains("network") {
            return "网络不可达：目标主机无法访问，请检查网络连接和防火墙设置".to_string();
        }
        
        // 检查是否是端口连接被拒绝
        if error_str.contains("refused") || error_str.contains("connection refused") {
            return "连接被拒绝：目标端口未开放或服务未启动".to_string();
        }
...
impl UnifiedTool for HttpRequestTool {
    ...
    async fn execute(&self, params: ToolExecutionParams) -> Result<ToolExecutionResult> {
        ...
        let url = params.inputs.get("url")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow!("参数错误：缺少必需的URL参数"))?;
        
        if url.is_empty() {
            return Err(anyhow!("参数错误：URL不能为空"));
        }
        
        // 验证URL格式
        if !url.starts_with("http://") && !url.starts_with("https://") {
            return Err(anyhow!("参数错误：URL必须以http://或https://开头，当前URL: {}", url));
        }
        ...
        if timeout_seconds == 0 || timeout_seconds > 300 {
            return Err(anyhow!("参数错误：超时时间必须在1-300秒之间，当前值: {}秒", timeout_seconds));
        }
        ...
        match self.execute_request(config).await {
            Ok(result) => {
                ...
                let status_info = if let Some(ref response) = result.response {
                    ...
                    format!("HTTP {} failed - {} ({})", result.request.method, result.request.url, error)
                } else {
                ...
                if result.success {
                    ...
                    error!("{} failed in {}ms", status_info, result.total_time_ms);
                }
                ...
                Ok(ToolExecutionResult {
                    ...
                    success: result.success,
                    output: result_json,
                    error: result.error.clone(), // 确保错误信息传递到工具执行结果
                    execution_time_ms,
