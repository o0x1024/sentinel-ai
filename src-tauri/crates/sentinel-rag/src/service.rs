...
use crate::rag::embeddings::{create_reranking_provider, RerankingManager};
...
impl RagService {
    ...
    pub async fn query_for_assistant(&self, request: &crate::rag::models::AssistantRagRequest) -> Result<(String, Vec<crate::rag::models::Citation>)> {
        ...
        // 使用 Rig + LanceDB 进行语义搜索
        let mut results: Vec<crate::rag::models::QueryResult> = match self.vector_store.search_similar(&collection_name, &request.query, top_k).await {
            Ok(results) => results,
            Err(e) => {
                warn!("向量搜索失败: {}, 返回空结果", e);
        ...
        if should_rerank {
            ...
            if let Some(reranker) = &self.reranker {
                match reranker.rerank(&request.query, &documents, Some(top_k)).await {
                    Ok(reranked) => {
                        let mut reordered: Vec<crate::rag::models::QueryResult> = Vec::new();
...
mod tests {
    ...
    async fn test_rag_service_creation() {
        ...
        let config = RagConfig {
            ...
            chunking_strategy: crate::rag::config::ChunkingStrategy::RecursiveCharacter,
