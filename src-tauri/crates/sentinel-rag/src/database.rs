...
impl LanceDbManager {
    ...
    pub async fn create_collection(&self, collection_name: &str, _embedding_dim: usize) -> Result<()> {
        // Lazy creation on first insert
        info!("Collection '{}' will be created on first insert", collection_name);
        Ok(())
    }
    ...
    pub async fn insert_chunks(&self, collection_name: &str, chunks: Vec<DocumentChunk>) -> Result<usize> {
        ...
        println!("embedding_model: {:?}", self.embedding_config.model.as_str());
        ...
        let dims = embeddings.first().map(|e| e.vec.len()).unwrap_or(0);
        if dims == 0 { return Err(anyhow!("Embedding dims is 0")); }
        let mut flat: Vec<f64> = Vec::with_capacity(definitions.len() * dims);
        ...
        let embedding_arr = Arc::new(FixedSizeListArray::try_new(
            Arc::new(Field::new("item", DataType::Float64, true)),
            dims as i32,
            flat_arr,
            None,
        ).map_err(|e| anyhow!(e.to_string()))?) as ArrayRef;
        ...
        let batch = RecordBatch::try_new(
            schema.clone(),
            vec![
                id_arr, source_id_arr, chunk_index_arr, definition_arr, embedding_arr,
                file_name_arr, file_path_arr, start_char_arr, end_char_arr,
            ],
        ...
        let conn = {
            ...
            guard.as_ref().cloned().ok_or_else(|| anyhow!("LanceDB not initialized"))?
        };

        // Try open table; if not exists, create
        let table: Table = match conn.open_table(collection_name).execute().await {
            Ok(t) => t,
            Err(_) => {
                let reader = arrow_array::RecordBatchIterator::new(vec![Ok(batch.clone())].into_iter(), schema.clone());
    ...
    pub async fn search_similar(&self, collection_name: &str, query: &str, top_k: usize) -> Result<Vec<QueryResult>> {
        ...
        for (rank, (score, _id, value)) in top_docs.into_iter().enumerate() {
            ...
            let end_char = value.get("end_char").and_then(|v| v.as_i64()).unwrap_or(0) as usize;
                ...
                chunk_index,
                        metadata: crate::rag::models::ChunkMetadata {
                    file_path,
                    file_name,
    ...
    pub async fn delete_collection(&self, collection_name: &str) -> Result<()> {
        ...
        let _ = conn.drop_table(collection_name).await; // ignore error if not exists
    ...
    pub async fn get_collection_stats(&self, collection_name: &str) -> Result<(usize, usize)> {
        ...
        let mut stream = table
            .query()
            .select(lancedb::query::Select::Columns(vec!["source_id".to_string()]))
            .execute()
            .await
            .map_err(|e| anyhow!(e.to_string()))?;
